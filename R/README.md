...
Code for the R package `EDI` and the code to reproduce the simulations in the papers listed in the [top-level README](../README.md). You can get started by cloning and running `R CMD INSTALL EDI` from this directory. Then try running the code in the folder `package_tests`.

> **Benchmark report:** [`benchmark/benchmark_model_fits_R.html`](benchmark/benchmark_model_fits_R.html) — speed and correctness of every model-fitting kernel against its R canonical baseline.

If you just want the model-fitting kernels from Python without an R
installation, see [`../python/`](../python/README.md) — a `pybind11` package (`edi_kernels`)

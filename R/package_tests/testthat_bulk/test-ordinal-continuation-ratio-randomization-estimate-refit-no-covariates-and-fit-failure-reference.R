...
# InferenceOrdinalContRatioRegr's compute_treatment_estimate_during_randomization_inference()
# (inference_ordinal_stereotype_logit.R) had no test reference anywhere -- the same gap pattern
# already closed this session on the sibling ordinal classes (cumulative-link family and adjacent-
# category logit). Independent reference is VGAM::vglm(cratio), the same reference already used for
# this class's compute_estimate() in test-ordinal-adjacent-category-continuation-ratio-and-gcomp-
# classes-match-vgam-and-polr-references.R. This class's source comment documents the same past bug
# pattern as its cumulative-link/adjacent-category siblings: using res$b[length(res$b)] instead of
# res$b[1] fed the randomization test the wrong coefficient -- fixed to res$b[1], verified here
# directly against an independent reference on a design WITH covariates.
#   1. The covariate-adjusted refit on the current (and then a permuted) w matches an independent
#      VGAM::vglm(family = cratio(parallel = TRUE)) fit.
...
cratio_fixture <- function(seed = 4L, n = 200L) {
	set.seed(seed)
	x <- rnorm(n)
...
cratio_no_cov_fixture <- function(seed = 5L, n = 200L) {
	set.seed(seed)
	des <- DesignFixediBCRD$new(n = n, response_type = "ordinal", verbose = FALSE)
	...
	y <- as.integer(cut(0.9 * w + rlogis(n), c(-Inf, -1, 0.4, 1.5, Inf)))
	des$add_all_subject_responses(y)
	inf <- InferenceOrdinalContRatioRegr$new(des, model_formula = ~1, verbose = FALSE)
	inf$num_cores <- 1L
	list(inf = inf, priv = inf$.__enclos_env__$private, y = y, w = w)
}

test_that("the randomization-time estimate refits the treatment coefficient for a permuted assignment, matching vglm(cratio)", {
	f <- cratio_fixture()
	...
	fit <- VGAM::vglm(y ~ w + x, family = VGAM::cratio(parallel = TRUE), data = d)
	...
	fit2 <- VGAM::vglm(y ~ w + x, family = VGAM::cratio(parallel = TRUE), data = d2)
...
test_that("with no covariates selected, the refit uses a treatment-only design and matches vglm(y ~ w, cratio)", {
	f <- cratio_no_cov_fixture()
	f$inf$compute_estimate()
	...
	fit <- VGAM::vglm(y ~ w, family = VGAM::cratio(parallel = TRUE), data = d)
	ref <- unname(VGAM::coef(fit)["w"])
	expect_equal(est, ref, tolerance = 1e-3)
})

test_that("a fitter failure (res NULL or non-finite res$b[1]) returns NA", {
	f <- cratio_fixture(seed = 7L)
	f$inf$compute_estimate()
	local_mocked_bindings(fast_continuation_ratio_regression_cpp = function(...) NULL, .package = "EDI")
	expect_true(is.na(f$priv$compute_treatment_estimate_during_randomization_inference()))

	local_mocked_bindings(
		fast_continuation_ratio_regression_cpp = function(X, ...) list(b = c(NA_real_, 0, 0, 0), fisher_information = diag(4)),
		.package = "EDI"
	)
	expect_true(is.na(f$priv$compute_treatment_estimate_during_randomization_inference()))
})

...
# InferencePropKKQuantileRegrOneLik$compute_estimate_with_bootstrap_weights() --
# the single-fit stacked quantreg estimator (matched-pair diffs + reservoir
# rows, both on the logit scale) -- had zero prior test-file references
# anywhere (only migration-golden coverage of its unrelated OneLik siblings).
# Verify the weighted refit by independently rebuilding the same stacked
# design from the class's own cached KKstats partition (a legitimate ground
# truth -- match/reservoir assignment is a design-level fact, not something
# under test here) and fitting quantreg::rq() directly, rather than reusing
# the private compute_weighted_combined_estimate() method itself.

make_prop_kk_qr_fixture <- function(seed, n = 60L){
	set.seed(seed)
	...
	w <- des$get_w()
	y <- plogis(-0.2 + 0.5 * w + 0.3 * rnorm(n))
...
independent_stacked_rq_beta <- function(inf, row_weights, tau = 0.5){
	...
	yd <- qlogis(KK$yTs_matched) - qlogis(KK$yCs_matched)
	Xd <- as.matrix(KK$X_matched_diffs_full)
	y_r <- qlogis(KK$y_reservoir)
	w_r <- KK$w_reservoir
...
test_that("weighted stacked quantreg refit matches an independently rebuilt reference", {
	des <- make_prop_kk_qr_fixture(20260918L)
	inf <- InferencePropKKQuantileRegrOneLik$new(des, verbose = FALSE)
	n <- des$get_t()
...
test_that("unit weights reproduce compute_estimate() via the effectively-constant shortcut", {
	des <- make_prop_kk_qr_fixture(20260919L)
	inf <- InferencePropKKQuantileRegrOneLik$new(des, verbose = FALSE)
	unweighted <- as.numeric(inf$compute_estimate(estimate_only = TRUE))

	inf2 <- InferencePropKKQuantileRegrOneLik$new(des, verbose = FALSE)
	n <- des$get_t()
...
test_that("genuinely varying weights diverge from the unweighted estimate", {
	des <- make_prop_kk_qr_fixture(20260920L)
	inf <- InferencePropKKQuantileRegrOneLik$new(des, verbose = FALSE)
	unweighted <- as.numeric(inf$compute_estimate(estimate_only = TRUE))

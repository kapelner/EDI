...
# InferenceCountPoisson's compute_treatment_estimate_during_randomization_inference()
# (inference_count_poisson.R) had no test reference anywhere -- test-rand-bootstrap.R already
# exercises it indirectly via BRT fast-kernel smoke tests, but nothing asserts on this method's own
# branches directly. Same shape as several sibling classes closed this session.
#   1. The covariate-adjusted refit on the current (and then a permuted) w matches an independent
#      glm(family = poisson()) fit exactly.
...
count_pois_fixture <- function(seed = 1L, n = 80L) {
	set.seed(seed)
	...
	w <- des$get_w()
	y <- rpois(n, exp(0.5 + 0.4 * w + 0.3 * X$x1))
...
count_pois_no_cov_fixture <- function(seed = 2L, n = 80L) {
	set.seed(seed)
	des <- DesignFixediBCRD$new(n = n, response_type = "count", verbose = FALSE)
	...
	des$add_all_subject_responses(y)
	inf <- InferenceCountPoisson$new(des, model_formula = ~1, verbose = FALSE)
	inf$num_cores <- 1L
	list(inf = inf, priv = inf$.__enclos_env__$private, y = y, w = w)
}

test_that("the randomization-time estimate refits the treatment coefficient for a permuted assignment, matching glm(poisson) exactly", {
	f <- count_pois_fixture()
...
test_that("with no covariates selected, the refit uses X = cbind(1, w) and matches glm(y ~ w, poisson) exactly", {
	f <- count_pois_no_cov_fixture()
	f$inf$compute_estimate()

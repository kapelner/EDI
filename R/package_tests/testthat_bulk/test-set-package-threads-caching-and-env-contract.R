...
test_that("set_package_threads() sets the documented env vars, mc.cores, and its own cache option", {
	on.exit({
	...
	expect_equal(getOption("mc.cores"), 3L)
	expect_equal(getOption(".edi_last_set_threads"), 3L)
})
...
test_that("set_package_threads() short-circuits (skips its own env-setting work) when re-called with an identical core count", {
	...
	expect_equal(getOption(".edi_last_set_threads"), 4L)
})
...
test_that("set_package_threads() coerces a non-integer num_cores and treats the coerced value as identical for caching", {
	...
	expect_equal(getOption(".edi_last_set_threads"), 2L)
	expect_true(is.integer(getOption(".edi_last_set_threads")))

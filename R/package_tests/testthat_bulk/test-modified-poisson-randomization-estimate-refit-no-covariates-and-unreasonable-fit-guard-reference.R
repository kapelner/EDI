...
# InferenceIncidModifiedPoisson's compute_treatment_estimate_during_randomization_inference()
# (inference_incidence_modified_poisson.R) had no test reference anywhere -- the same gap pattern
# already closed this session on InferenceIncidLogRegr/InferenceIncidProbitRegr/InferenceCountNegBin,
# but with an EXTRA guard: two distinct NA-return sites, not one.
#   1. The covariate-adjusted refit on the current (and then a permuted) w matches an independent
#      glm(family = poisson()) fit (modified/robust Poisson's point estimate is the ordinary Poisson
#      MLE; only its variance is sandwich-corrected, which this fast estimate_only path skips).
...
#   4. res NULL or a non-finite res$b[2] (the fitter itself fails) returns NA.
#   5. A DISTINCT site: a finite res$b[2] that still fails is_modified_poisson_fit_reasonable() also
#      returns NA.

modpois_fixture <- function(seed = 1L, n = 80L) {
	set.seed(seed)
...
modpois_no_cov_fixture <- function(seed = 2L, n = 80L) {
	set.seed(seed)
	des <- DesignFixediBCRD$new(n = n, response_type = "incidence", verbose = FALSE)
	...
	des$add_all_subject_responses(y)
	inf <- InferenceIncidModifiedPoisson$new(des, model_formula = ~1, verbose = FALSE)
	inf$num_cores <- 1L
...
test_that("the randomization-time estimate refits the treatment coefficient for a permuted assignment, matching glm(poisson)", {
	...
	ref <- unname(coef(glm(f$y ~ f$w + f$X$x1, family = poisson()))[2])
	...
	ref2 <- unname(coef(glm(f$y ~ w2 + f$X$x1, family = poisson()))[2])
...
test_that("a finite-but-unreasonable fit (is_modified_poisson_fit_reasonable() FALSE) also returns NA -- distinct site", {
	f <- modpois_fixture(seed = 5L)
	f$inf$compute_estimate()
	unlockBinding("is_modified_poisson_fit_reasonable", f$priv)
	f$priv$is_modified_poisson_fit_reasonable <- function(...) FALSE

expect_w_encoding = function(w, n = NULL) {
  expect_true(all(w %in% c(0L, 1L)),
    info = paste("w contains values outside {0, 1}:", paste(unique(w), collapse = ", ")))
  if (!is.null(n)) expect_length(w, n)
}

expect_ws_encoding = function(ws, n = NULL, r = NULL) {
  expect_true(all(ws %in% c(0L, 1L)),
    info = paste("ws matrix contains values outside {0, 1}:", paste(unique(as.vector(ws)), collapse = ", ")))
...
test_that("DesignSeqOneByOneiBCRD get_w returns {0,1}", {
  set.seed(1)
  des = run_seq_design(DesignSeqOneByOneiBCRD$new(n = n, response_type = "continuous"), X)
  expect_w_encoding(des$get_w(), n)
})

test_that("DesignSeqOneByOnePocockSimon get_w returns {0,1}", {
  set.seed(1)
  des = run_seq_design(
...
test_that("DesignSeqOneByOneUrn get_w returns {0,1}", {
  set.seed(1)
...
test_that("DesignFixedBernoulli get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixedRerandomization get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixedBlocking get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixedGreedy get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixedOptimalBlocks get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixediBCRD get_w and draw_ws return {0,1}", {
  set.seed(1)
...
test_that("DesignFixedCluster get_w and draw_ws return {0,1}", {
  set.seed(1)
  des = run_fixed_design(
...
test_that("DesignFixedBlockedCluster get_w and draw_ws return {0,1}", {
  set.seed(1)
  des = run_fixed_design(

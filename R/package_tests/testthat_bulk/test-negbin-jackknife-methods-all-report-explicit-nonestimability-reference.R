...
# InferenceCountNegBin's five compute_jackknife_*() overrides (estimate, bias_estimate, std_error,
# wald_two_sided_pval, wald_confidence_interval): the class's own documentation states delete-one refits
# of the negative-binomial model are numerically unstable, so every one of these always short-circuits to
# NA and flags "negbin_jackknife_not_supported" -- never attempting a refit. The identical pattern on the
# sibling InferenceCountHurdleNegBin class is now covered elsewhere in this suite; this single-part
# InferenceCountNegBin's own five overrides had no test anywhere calling them.
...
fx <- function(seed = 1L, n = 40L) {
	...
	InferenceCountNegBin$new(d, verbose = FALSE)
}
...
test_that("every jackknife method returns NA (or an all-NA CI) and flags the same explicit non-estimability reason", {
	...
	expect_identical(inf$get_nonestimable_reason(), "negbin_jackknife_not_supported")
	expect_true(inf$is_nonestimable("se"))

	inf2 <- fx()
	expect_true(is.na(inf2$compute_jackknife_bias_estimate()))
	expect_identical(inf2$get_nonestimable_reason(), "negbin_jackknife_not_supported")

	inf3 <- fx()
	expect_true(is.na(inf3$compute_jackknife_std_error()))
	expect_identical(inf3$get_nonestimable_reason(), "negbin_jackknife_not_supported")

	inf4 <- fx()
	expect_true(is.na(inf4$compute_jackknife_wald_two_sided_pval(delta = 0.5)))
	expect_identical(inf4$get_nonestimable_reason(), "negbin_jackknife_not_supported")

	inf5 <- fx()
	ci <- inf5$compute_jackknife_wald_confidence_interval(alpha = 0.1)
	expect_true(all(is.na(ci)))
	expect_identical(names(ci), c("5%", "95%"))
	expect_identical(inf5$get_nonestimable_reason(), "negbin_jackknife_not_supported")
})

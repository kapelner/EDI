...
# Keep these HL calls serial: their median helpers currently check R
# interrupts inside OpenMP workers, which can abort R with two cores.
...
test_that("KK HL bootstrap discards incomplete and same-arm pairs while retaining the reservoir", {
	...
	expect_equal(EDI:::compute_wilcox_matching_ivwc_bootstrap_parallel_cpp(w, y, integer(8), indices, m, 1L), c(median(outer(c(5, 8), c(1, 3), "-")), NA))
})

...
# InferenceCountRobustPoisson's compute_treatment_estimate_during_randomization_inference()
# (inference_count_robust_poisson.R) had no test reference anywhere -- the same gap pattern already
# closed this session on the near-identical sibling InferenceCountQuasiPoisson (the robust/quasi
# Poisson point estimate is the ordinary Poisson MLE; only the sandwich-corrected SE differs, which
# this fast estimate_only path skips entirely). The only source difference from the QuasiPoisson
# sibling is the tryCatch's is_edi_control_condition() re-throw guard, not exercised by these tests.
...
robpois_fixture <- function(seed = 1L, n = 80L) {
	set.seed(seed)
	x <- rnorm(n)
	...
	inf <- InferenceCountRobustPoisson$new(des, verbose = FALSE)
	inf$num_cores <- 1L
	list(inf = inf, priv = inf$.__enclos_env__$private, x = x, y = y, w = w)
}

robpois_no_cov_fixture <- function(seed = 2L, n = 80L) {
	set.seed(seed)
	des <- DesignFixediBCRD$new(n = n, response_type = "count", verbose = FALSE)
	...
	des$add_all_subject_responses(y)
	inf <- InferenceCountRobustPoisson$new(des, model_formula = ~1, verbose = FALSE)
	inf$num_cores <- 1L

...
test_that("fast_beta_regression_cpp argument permutations", {
	...
	res3 <- EDI:::fast_beta_regression_cpp(X, y, compute_std_errs = FALSE)
...
test_that("fast_weibull_regression robustness", {
	...
	expect_error(EDI:::fast_weibull_regression(y, dead, X), NA)
})
...
test_that("create_design_matrix consistency", {
	# We test this by initializing an inference object and checking the private$m method
	set.seed(1)
	n <- 20
...
test_that("fast_beta_regression_with_var consistency", {
	...
	res <- EDI:::fast_beta_regression_with_var(Xmm, y)

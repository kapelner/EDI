...
audit_classes = list(
  ...
  list(name="InferenceSurvivalKKWeibullFrailtyNormalOneLik", section="Survival",  resp="surv", kk=TRUE,  types="full", skip_asymp=FALSE, skip_ci=FALSE,   skip_boot=TRUE,  skip_bbt=TRUE,  jack=TRUE, skip_rand=FALSE, rand_resp="csp", skip_rpv=FALSE, skip_rci=FALSE, rci_resp="surv", pboot=TRUE,  slow_methods=c("compute_rand_confidence_interval", "compute_score_confidence_interval"), notes="AbstractKKWeibullFrailtyNormalOneLik → ParamBootstrap; pboot=use_rcpp; skip_boot=TRUE; rand CI avg 45.7s / max 171.5s at n=6; score CI avg 50.1s / max 324.8s at n=13 slow"),
...
html_from_audit = function(classes, outfile = "path_audits.html") {
  ...
  # means skipped in comprehensive tests -- either for runtime (InferenceSurvivalKKWeibullFrailtyLoggammaOneLik,

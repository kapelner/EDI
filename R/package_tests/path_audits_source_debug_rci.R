...
html_from_audit = function(tables, outfile = "path_audits.html") {
  ...
  method_cell = function(r, method_id, default_status) {
    .RCI_TARGETS <- c(
      "InferenceIncidKKCondLogitOneLik","InferenceIncidKKCondLogitGLMMOneLik","InferenceIncidKKGEE",
      "InferenceIncidKKNewcombeRiskDiff","InferenceIncidKKGCompRiskDiff","InferenceIncidKKGCompRiskRatio",
      "InferenceIncidKKModifiedPoisson","InferenceIncidWald","InferenceIncidLogRegr","InferenceIncidProbitRegr",
      "InferenceIncidMiettinenNurminenRiskDiff","InferenceIncidNewcombeRiskDiff","InferenceIncidRiskDiff",
      "InferenceIncidGCompRiskDiff","InferenceIncidGCompRiskRatio","InferenceIncidModifiedPoisson",
      "InferenceIncidLogBinomial","InferenceIncidBinomialIdentityRiskDiff","InferenceCountKKHurdlePoissonOneLik",
      "InferenceCountKKCondPoissonOneLik","InferenceCountPoisson","InferenceCountRobustPoisson",
      "InferenceCountQuasiPoisson","InferenceCountNegBin","InferenceCountZeroInflatedNegBin",
      "InferenceCountHurdlePoisson","InferenceCountHurdleNegBin","InferenceOrdinalGCompMeanDiff","InferenceOrdinalRidit")
    if (r$name %in% .RCI_TARGETS && grepl("confidence_interval", method_id) && grepl("rand", method_id)) {
      .st <- method_status(r, method_id, default_status)
      .RCI_LOG[[length(.RCI_LOG) + 1]] <<- list(name = r$name, formula = r$formula %||% NA, method_id = method_id, rci_resp = r$rci_resp %||% NA, default_status = default_status, resolved_status = .st)
    }

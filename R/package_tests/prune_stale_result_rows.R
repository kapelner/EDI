...
cat(sprintf("prune_stale_result_rows: last source-touching commit is %s\n", substr(last_src_commit, 1L, 10L)))
total_stale = 0L
for (f in files) {
	probe = fread(f, select = c("status", "github_commit_id"), showProgress = FALSE)
	...
	n_stale = sum(is_stale)
	total_stale = total_stale + n_stale
	cat(sprintf("  %-52s rows=%9d error=%9d stale-error=%9d\n", basename(f), nrow(probe), sum(probe$status == "error"), n_stale))
	if (apply_changes && n_stale > 0L) {
		full = fread(f, showProgress = FALSE)
		tmp = paste0(f, ".prune_tmp")
		fwrite(full[!is_stale], tmp, quote = TRUE, showProgress = FALSE)
		if (!file.rename(tmp, f)) stop("could not replace ", f)
		cat(sprintf("    removed %d rows -> %d remain\n", n_stale, nrow(full) - n_stale))
	}
}
if (!apply_changes && total_stale > 0L) cat(sprintf("prune_stale_result_rows: %d stale error row(s) would be removed; re-run with --apply.\n", total_stale))

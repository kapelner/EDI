...
Structural note (2026-08-13): this lands as a registered inference component
(e.g. `MOutOfNBootstrap`, `provides_capabilities = "m_out_of_n_bootstrap"`,
`load_policy = "lazy"`), not as methods added to a shared bootstrap base —
the old `InferenceNonParamBootstrap -> ... -> InferenceJackknife` ladder is
deleted by `fix_inference_hierarchy.md`. The public methods above are
registered under the capability in `public_methods_for_capability`, classes
opt in via `components = c(...)` in `define_inference_class()`, and the
loader/estimator hooks in the operation contract are declared through the
component contract (same shape as the `Subsampling` component in
`prw_subsampling_implementation_spec.md`, which shares the
exchangeable-unit resolver).
...
feature is in fact implemented — `R/inference_ext_m_out_of_n_bootstrap.R` (impl
methods), public wrappers on the nonparametric-bootstrap surface
(`inference_all_abstract_non_param_boot.R:170-252+`), operation-contract entries
in `resampling_draw_contracts.R`, the shared exchangeable-unit resolver
(`inference_ext_exchangeable_resampling_units.R`), and tests
(`test-m-out-of-n-prw-subsampling.R`). Checkboxes below updated to match
verified code; Phase 4 remains genuinely open.

### Phase 1: Core m-out-of-n Bootstrap

- [x] TODO-1: Add operation contract and private cache.
- [x] TODO-2: Add ordinary unit-level with-replacement size-`m` draw generator.
- [x] TODO-3: Add public distribution, p-value, and CI methods.
  (`approximate_m_out_of_n_bootstrap_distribution_beta_hat_T`,
  `compute_m_out_of_n_bootstrap_two_sided_pval`,
  `compute_m_out_of_n_bootstrap_confidence_interval`.)
- [x] TODO-4: Support explicit integer `m` and deterministic `m = NULL`.
- [x] TODO-5: Support only `scaling = "sqrt_n"`.

### Phase 2: Shared Minimum-Volatility Selector

- [x] TODO-6: Add generic `select_optimal_resample_size()`.
- [x] TODO-7: Add `select_optimal_m_out_of_n_bootstrap()`.
- [x] TODO-8: Reuse the same selector engine later from
  `select_optimal_b_subsampling()`. (PRW's selector calls the shared
  `private$select_optimal_resample_size()`.)
- [x] TODO-9: Add selector tests. (`test-m-out-of-n-prw-subsampling.R`.)

### Phase 3: Design-Aware Units

- [x] TODO-10: Add cluster, blocking, matching, and KK matched/reservoir draw support.
  (Via the shared `get_exchangeable_units()` resolver.)
- [x] TODO-11: Keep replacement semantics explicit in tests. (Closed 2026-08-14:
  added the `"replacement semantics are explicit"` test to
  `test-m-out-of-n-prw-subsampling.R` — five m = n = 20 with-replacement draws
  must contain a duplicated `i_b` row (P(no duplicate) ≈ 2e-8 per draw), while
  twenty PRW `b = 12` draws must each be duplicate-free with exactly `b` rows.
  Full file passes: 103 assertions, 0 failures.)

### Phase 4: Diagnostics And Debug API

Closed out 2026-08-14: what this spec can deliver on its own is done — the
implementation exposes per-distribution diagnostics via `debug = TRUE`
(resolved `m`, finite fraction, failure reasons; see
`approximate_m_out_of_n_bootstrap_distribution_beta_hat_T_impl` and
`evaluate_m_out_of_n_bootstrap_size` in
`inference_ext_m_out_of_n_bootstrap.R`). The two items below depend on the
public debug API that `public_diagnostics_api_spec.md` owns and has not built
yet, so they are re-homed there (its TODO-17/TODO-18) rather than held open
here.

- [x] TODO-12: Wire m-out-of-n diagnostics into the public debug API.
  (Re-homed to `public_diagnostics_api_spec.md` TODO-17; `debug = TRUE`
  diagnostics already exist here to wire in.)
- [x] TODO-13: Preserve optimizer diagnostics for failed refits in debug mode when
  available. (Re-homed to `public_diagnostics_api_spec.md` TODO-18; depends on
  that spec's Phase 3 optimizer-diagnostics layer.)

### Phase 5: Path Audit Integration

- [x] TODO-14: Add separate path-audit columns for m-out-of-n p-values and CIs.
  (`path_audits_source.R`: `slow_m_out_of_n_methods` + comprehensive-tests wiring.)
- [x] TODO-15: Do not fold m-out-of-n into ordinary bootstrap columns.
  (Tracked as its own method group in `path_audits_source.R`.)

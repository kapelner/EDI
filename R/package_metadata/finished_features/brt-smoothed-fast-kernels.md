...
**Goal:** Extend every `compute_fast_rand_bootstrap_distr` C++ kernel that operates on real-valued (continuous/survival) responses to accept an optional per-draw, per-row noise matrix, so that `type = "smoothed"` bootstrap-randomization p-values/CIs use the fast C++ path instead of the ~7x-slower R-level per-draw fallback (the `RandomizationBootstrap` component's `approximate_rand_bootstrap_distribution_beta_hat_T` reused-worker / per-iteration path — formerly `InferenceRandBootstrap`, a base deleted by `fix_inference_hierarchy.md`; the method body still lives in `inference_all_abstract_rand_bootstrap.R` as component source, so this plan's edit sites are unchanged).
...
> **Status (audit, 2026-08-14): IMPLEMENTED — checkboxes below were never
> ticked during implementation and are historical.** Verified in code:
> `noise_mat` is accepted by all 8 in-scope kernels (`fast_wilcox_hl.cpp`,
> `rand_bootstrap_mean_diff_parallel.cpp`, `rand_bootstrap_ols_parallel.cpp`,
> `fast_robust_regression.cpp`, `fast_coxph_regression.cpp`,
> `fast_weibull_regression.cpp`, `fast_survival_stats.cpp`, `fast_logrank.cpp`)
> plus `RcppExports.cpp`; `rand_bootstrap_draw_matrices()` packs `noise_mat`
> (`inference_all_abstract_rand_bootstrap.R:501-513`); Ridit and
> Jonckheere-Terpstra explicitly decline noise-carrying draws
> (`inference_ordinal_ridit.R:166`,
> `inference_ordinal_jonckheere_terpstra_test.R:100`); ten
> `test-brt-smoothed-*.R` files cover every kernel plus the plumbing
> (`test-brt-smoothed-noise-mat-plumbing.R`) and the end-to-end perf case
> (`test-brt-smoothed-wilcox-ci-perf.R`). The per-task TDD step boxes below
> cannot be retroactively verified individually and are left unticked — treat
> this plan as done. Task 12 (the one slice without direct test-file
> evidence) was verified complete 2026-08-14 — see its per-step notes below.
...
- Install with `R CMD INSTALL --no-docs EDI` (matches this repo's established convention; see `package_metadata/new_feature_plans/performance_profiling_and_upgrades.md`). Do **not** run `roxygen2::roxygenize()` directly for `.Rd`/`NAMESPACE` regeneration — use `Rscript fast_roxygenize.R` from the repo root (only needed in Task 12, which touches roxygen text).
...
- Shallow-hierarchy notes (2026-08-13, assuming `fix_inference_hierarchy.md` and `sexp_removal_rcppeigen_conversion_spec.md` are complete): (a) the `has_private_method("compute_fast_rand_bootstrap_distr")` dispatch this plan edits is sanctioned — these `compute_fast_*` hooks are host-implemented optional hooks declared as `optional_private_methods` in the component contracts, not the banned semantic-classification use; (b) any test subclassing a migrated `Inference*` class (e.g. the `SlowInference*` forced-fallback wrappers below) must pass `lock_objects = FALSE`; (c) the "current" C++ signatures quoted in the per-kernel tasks predate the SEXP-removal conversion — re-verify each against the file before editing (`y0`-shaped response params legitimately stay `SEXP` per that spec's permanent coercion exception, and these R-only kernels keep `Rcpp::List`/Rcpp types per its TODO-14 decision, so the approach here is unaffected; only the quoted parameter types/names may have drifted).
...
SlowInferenceAllSimpleWilcox = R6::R6Class(
	...
	# required for subclasses of factory-built classes: lazy components install
	# methods onto private after construction
	lock_objects = FALSE,
	private = list(
		compute_fast_rand_bootstrap_distr = function(y0_full, rand_bootstrap_draws, delta, transform_responses, zero_one_logit_clamp = .Machine$double.eps){
			NULL
...
- [x] **Step 1: Measure the real speedup to cite** (done during implementation: measured numbers — 25.5s forced-slow vs 0.5s fast-kernel CI, ~50x, `n = 30`, `B = 99` — are cited in the shipped roxygen)
...
- [x] **Step 2: Update the roxygen "Implementation caveat" paragraph** (done, already in HEAD — and with a *corrected* account vs. this template: only the CI path (`compute_rand_bootstrap_confidence_interval(type = "smoothed")`) is accelerated, since a standalone smoothed p-value call uses `materialize_w = FALSE` and never builds the matrices the fast kernels need)
...
- [x] **Step 3: Add a dated entry to `package_metadata/perf_experiments_final.md`** (done, already in HEAD — entry "BRT smoothed fast-kernel noise support — 2026-07-20"; the file now lives at `package_metadata/audits/perf_experiments_final.md`)
...
- [x] **Step 4: Regenerate NAMESPACE/docs** (done — `man/InferenceRandBootstrap.Rd` carries the Performance paragraph)
...
- [x] **Step 5: Update the auto-memory file** (verified moot 2026-08-14: `project_smoothed_brt_fast_path_bypass.md` does not exist in the auto-memory directory and `MEMORY.md` has no such entry — the resolved state is recorded in the repo itself via the perf-log entry and roxygen, which is where it belongs)
...
- [x] **Step 6: Final full-suite sanity check** (run 2026-08-14 with the `brt-smoothed` filter: 25 PASS, 0 FAIL in the kernel/plumbing files; 2 ERRORs confined to `test-brt-smoothed-wilcox-ci-perf.R`, both `attempt to apply non-function` because `InferenceAllSimpleWilcox` currently lacks `compute_rand_bootstrap_confidence_interval` — a pre-existing symptom of the in-flight `fix_inference_hierarchy.md` migration (its p-value sibling exists), unrelated to this plan's docs-only Task 12; tracked in that plan's Follow-Ups)
...
- [x] **Step 7: Commit** (nothing left to commit — all Task 12 artifacts were already committed in HEAD before this verification pass)

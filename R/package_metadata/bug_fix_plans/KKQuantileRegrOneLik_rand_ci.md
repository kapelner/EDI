...
> `../new_feature_plans/incidence_randomization_cis.md` and `../new_feature_plans/randomization_ci_construction_audit.md` --
...
`../new_feature_plans/incidence_randomization_cis.md`'s stopgap. `compute_rand_two_sided_pval()` is
...
2. **Real bespoke fix: a stacked-model Zhang-style permutation test.**
   ...
   *exactness* (no generic Monte Carlo bisection precision questions,
   `../new_feature_plans/randomization_ci_search_precision.md`'s open track) at the cost of real
   ...
   `../new_feature_plans/incidence_randomization_cis.md`'s TODO-3 for the same caveat on a
   sibling problem).
...
4. If option 2: derive the stacked-model exact/permutation shift model and
   ...
   any C++ kernel, same caveat as `../new_feature_plans/incidence_randomization_cis.md`'s TODO-3.
...
6. Cross-reference this plan from `release_v1_1_0.md`/`../new_feature_plans/_master.md` (done,
   `TODO-25`).
...
- The TODO-3 regression test (non-degenerate width + coverage sanity check)
  ...
  reappear," the same way `../new_feature_plans/incidence_randomization_cis.md`'s analogous TODO

...
> **Depends on:** `optimizer_diagnostics_report.md` Phase 2 (SolverDiagnostics classifier for the optim-based fits Firth needs); shares the penalized-testing-semantics decision with `l1_l2_penalties_all_likelihood_paths_report.md` — make it once, jointly. (Global ordering: see `_master.md`.)

> **Update (2026-08-14).** This report predates the completed
> `fix_inference_hierarchy.md` shallow-hierarchy migration. Its concrete class
> names survive, but the `supports_fisher_information()` override-gating
> discussed below is the retired `supports_*`-hook pattern: under the completed
> hierarchy, "which classes can provide Fisher information" is a registered
> capability queried via `obj$supports(...)`, and any package-wide Firth layer
> lands as a registered `InferenceComponent` composed through
> `define_inference_class()`, not as base-class methods or mixin splices.
...
| `InferenceSurvivalGLMMWeibullFrailtyNormalOneLik` | `fast_weibull_frailty_cpp` | **No** | Frailty integration and variance parameter make analytic Firth support impractical. |
| `InferenceSurvivalGLMMWeibullFrailtyLoggammaOneLik` | `fast_clayton_weibull_aft_optim_cpp` | **No** | Copula dependence parameter plus Weibull margins plus combined design structure. |
...
- [ ] TODO-2: If pursued: implement Strategy 1 (per-family analytic Firth-adjusted score + information + constrained null fits) for the limited rollout the report calls realistic — logistic, Poisson, maybe log-binomial; register as a component/capability, not `supports_*` overrides.
- [ ] TODO-3: Settle the information-matrix consistency question (expected vs. observed) per family before any code — the report flags this as a real correctness decision, now queryable via the capability metadata rather than `supports_fisher_information()` overrides.
- [ ] TODO-4: Decide the testing-layer semantics (what happens to score/LR/gradient tests under a penalized fit) before rollout; do not bundle with `l1_l2` ridge work, whose Phase-2 decision is the same question.
- [ ] TODO-5: Reject Strategy 2 (generic information-penalized objective) unless new evidence emerges, per the report.

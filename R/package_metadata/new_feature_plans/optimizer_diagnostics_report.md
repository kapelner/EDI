...
- [ ] TODO-1: Phase 1 — thread the two free diagnostics (gradient norm, GLMM variance-boundary flag) through the native fitter returns — as flat `diag_*` `edi::ResultMap` entries or wrapper-layer additions per this report's 2026-08-14 update note (no nested lists through the core); centralize the `1e6` separation threshold into one shared function (five call sites); decide the information-matrix minimum-eigenvalue cost question explicitly (fallback-path-only vs. universal `O(p^3)`).
- [ ] TODO-2: Phase 1b — wire the diagnostic classifier into `InferenceOrdinalStereotypeLogitRegr`, `InferenceOrdinalKKCondAdjCatLogitRegr`, `InferenceIncidKKCondLogitOneLik` (huge-finite-SE rejection).
- [ ] TODO-3: Phase 2 — build the `SolverDiagnostics` registered `InferenceComponent` (not a Pattern-1 mixin) classifying Layer-1 output into the standard taxonomy, routed through `cache_nonestimable_estimate()` with standardized reason strings; benchmark dispatch overhead before enabling inside CI-inversion/bootstrap/permutation inner loops (default: final reported fit only).
- [ ] TODO-4: Phase 3 — redefine `converged` as gradient-norm-based with `hit_iteration_cap` separate; audit every caller branching on `converged`.
- [ ] TODO-5: Phase 4 — quadrature-adequacy and always-on rank-deficiency visibility as separate, lower-priority projects.
- [ ] TODO-6: Then unblock `public_diagnostics_api_spec.md`'s TODO-9..12, which consume `get_last_fit_diagnostics()` from this layer.

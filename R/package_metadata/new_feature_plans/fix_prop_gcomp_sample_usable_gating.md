...
Unrelated mechanism to `fix_stale_worker_cache_resampling.md` — that plan
...
moving toward (`fix_stale_worker_cache_resampling.md`'s TODO-9 follow-up
...
- [ ] TODO-6: Regenerate any `comprehensive_tests` CSV rows for this class's
  ...
  `fix_stale_worker_cache_resampling.md`'s TODO-7 — only after install, not
  before).

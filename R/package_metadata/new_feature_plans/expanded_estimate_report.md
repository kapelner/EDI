...
> target quantity) — deliberately kept off this plan's `estimate_type` axis:
> `estimate_type` values are different *estimators of the same estimand*
> (bias-correction variants), while the estimand is *which quantity is
> targeted*. Decide the two jointly (its TODO-1 is spliced with this plan's
> TODO-1 in `_master.md`) so neither enum absorbs the other's values.
...
> interval-censored-survival plans). The class this report's title asks about no longer exists as a class:
> `InferenceParamBootstrap` is now the `ParametricLikelihoodBootstrap`
> component, so "grow a field on the class" translates to "add the option to
> the component's methods/capability metadata", and the
> `supports_lik_ratio_param_bootstrap()` gating cited below is now the
> `parametric_likelihood_bootstrap` capability read. The design question and
> recommendation are otherwise unchanged.
...
- [ ] TODO-4: Resolve open question 2: keep the dedicated `compute_param_bootstrap_*` methods as thin wrappers or deprecate them once the switch exists.
- [ ] TODO-5: Keep Firth off this axis (it is a different fitting method, not a post-fit transform), per the report's recommendation.

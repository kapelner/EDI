...
> **Depends on:** phases shipped. Remaining `fast_truncated_negbin_count_cpp` heap-corruption fix has no dependencies (batch it with `sexp_removal_rcppeigen_conversion_spec.md` TODO-16, the same class of C++ memory-safety bug). (Global ordering: see `_master.md`.)
...
> interval-censored-survival plans). `InferenceAsympLik`'s likelihood plumbing now lives in the `LikelihoodTests` component (gated by `likelihood_tier` metadata), `InferenceParamBootstrap` is the `ParametricLikelihoodBootstrap` component, `InferenceAsympLikStdModCache` is the `StandardModelCache` component, and `supports_*()` hook methods are capability reads (`obj$supports(...)`). Optional behavior is composed as registered `InferenceComponent`s via `define_inference_class()` — the deleted inheritance ladder's diagrams and "lives in class X" statements below should be read as "provided by the matching component". Additionally, the gap this report scoped has since been
> **implemented**: the null-simulation machinery (`simulate_under_lik_null()`
> hooks) and bootstrap-calibrated LR (`lik_ratio_bartlett_approx` and the
> parametric-bootstrap LR plumbing) shipped — the `parametric_likelihood_bootstrap`
> capability now requires exactly the hooks this report proposed
> (`get_likelihood_test_spec`, `simulate_under_lik_null`). The Easy/Borderline/
> Difficult per-family audit below remains the useful part; its structural
> "where should this live" sections are settled history.
...
| `InferenceSurvivalGLMMWeibullFrailtyNormalOneLik` | `fast_weibull_frailty_cpp` | **Difficult** | Frailty simulation plus censoring plus repeated null refits is too heavy for a first generic rollout. Remains a direct `InferenceAsympLik` child. |
| `InferenceSurvivalGLMMWeibullFrailtyLoggammaOneLik` | `fast_clayton_weibull_aft_optim_cpp` | **Difficult** | Copula-based survival simulation is possible in principle but too bespoke package-wide. Remains a direct `InferenceAsympLik` child. |
...
| `InferenceSurvivalGLMMWeibullFrailtyNormalOneLik` | Supported now | Rcpp-backed only; frailty simulation implemented behind `use_rcpp`. |
| `InferenceSurvivalGLMMWeibullFrailtyLoggammaOneLik` | Supported now | Clayton-copula survival simulation implemented. |
...
- [x] TODO-1: Fix the C-level heap corruption in `fast_truncated_negbin_count_cpp` that forced `InferenceCountHurdleNegBin` to ship with `supports_lik_ratio_param_bootstrap = FALSE` (see the Phase-3 status table) and re-enable that family — a real memory-safety bug, same class as the `fast_poisson_glmm_cpp` group-size crash tracked as TODO-16 in `sexp_removal_rcppeigen_conversion_spec.md`.
      **Done 2026-08-16.** Root cause confirmed by direct reproduction: not
      `fast_truncated_negbin_count_cpp`'s own entry point (which already
      validates `X`/`y`/`warm_start_params`/`warm_start_fisher_info` via
      `validate_truncated_negbin_inputs`) but its exported getter-style
      siblings `get_hurdle_negbin_count_score_cpp` and
      `get_hurdle_negbin_count_hessian_cpp` in `fast_hurdle_negbin.cpp` —
      exactly TODO-16's bug shape, just in the truncated-negbin file instead
      of the GLMM files that audit covered. Neither getter checked
      `X.rows() == y.size()` or `params.size() == ncol(X) + 1` before calling
      `TruncatedNegBinCount::operator()`, which does `params.head(m_p)` on
      the raw `Eigen::Map<Eigen::VectorXd>` — with `-DNDEBUG` forced package-
      wide (disabling Eigen's own bounds asserts), a too-short `params`
      reads past the end of that buffer. Reproduced directly: built a
      standalone `EDI_CORE_ONLY` (Rcpp-free) AddressSanitizer harness driving
      this exact call path outside R, and calling it with a deliberately
      too-short `params` produced a confirmed
      `AddressSanitizer: heap-buffer-overflow` `READ` at
      `fast_hurdle_negbin.cpp:337` (`params.head(m_p)`), matching the
      "double free or corruption" symptom class TODO-16 documented for the
      sibling GLMM getters. (Extensive fuzzing of
      `fast_truncated_negbin_count_cpp` itself — dimension-mismatched warm
      starts, thousands of repeated same-size refits, and CI-bisection-style
      repeated `fixed_idx`-constrained null refits with varying positive-row
      counts, all under ASan/UBSan — found nothing there, consistent with
      its existing validation being sound.) Fixed by adding the same
      `X.rows() == y.size()` / `params.size() == ncol(X) + 1` guards
      (`Rcpp::stop(...)`, matching `get_poisson_glmm_score_cpp`'s exact
      message style) to both getters in `fast_hurdle_negbin.cpp`. Verified
      the fix under the same ASan harness: the malformed-input call now
      throws cleanly instead of corrupting memory, and a valid,
      dimension-consistent call still returns the correct-sized
      score/hessian. Added a regression test
      (`test-truncated-negbin-hardening.R`) covering both getters'
      dimension-mismatch rejection and valid-call behavior. No R-level
      `supports_lik_ratio_param_bootstrap` flag needed flipping —
      `InferenceCountHurdleNegBin` already inherits `TRUE` from
      `InferenceCountLikelihood` (`inference_all_abstract_count_likelihood.R`);
      the family was unreliable in practice because of this crash, not
      gated by a disabled flag, so fixing the crash is the full re-enable.
- [ ] TODO-2: Make a decision about whether to attempt any Difficult-tier family (Cox/stratified Cox, frailty, copula, clogit+GLMM hybrids) — ask the user; the report recommends against a package-wide rollout.

...
> **Depends on:** `nominal_response_type_report.md` (Stage 1 is literally shared; single discrete choice rides on the nominal machinery). (Global ordering: see `_master.md`.)
...
- [ ] TODO-2: Stage 1 — admit single discrete choice as `nominal` (shared with `nominal_response_type_report.md`'s Stage 1; do these together).
- [ ] TODO-3: Stage 2 — conditional-logit variant for menu-varying discrete choice.
- [ ] TODO-4: Stage 3 — full/partial rankings (Plackett-Luce / Mallows) **only if** rankings are a real demand — new `Design` response shape + new C++ kernel; treat as its own gated project per the report's split verdict.

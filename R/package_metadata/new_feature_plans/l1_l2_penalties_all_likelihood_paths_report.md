...
> **Depends on:** shares its Phase-2 inference-semantics decision with `firth_penalties_report.md` (decide jointly). (Global ordering: see `_master.md`.)
...
- [ ] TODO-2: Phase 1 — ridge only, clean native single-likelihood models (logistic, Poisson, log-binomial, negative binomial, ordinary ordinal, Weibull, Cox), treatment coefficient unpenalized by default.
- [ ] TODO-3: Phase 2 — record the inference-semantics decision (penalized objective for tests vs. unpenalized tests with penalized nuisance fitting) before touching more families; shared with `firth_penalties_report.md`'s testing-layer question.
- [ ] TODO-4: Phase 3 — extend ridge to structured models (zero-augmented counts, GLMMs, frailty/copula) only after Phases 1-2 are stable. L1 remains out of scope per the report.

...
> **Depends on:** `expanded_estimate_report.md` (API-shape decision); pairs with `bias_correction_cox_snell.md`. (Global ordering: see `_master.md`.)
...
- [ ] TODO-2: Phase 1 — explicit estimate API: `compute_bias_corrected_estimate()` / `_wald_two_sided_pval()` / `_wald_confidence_interval()`; never silently redefine the existing estimate. Coordinate with `expanded_estimate_report.md`'s `estimate_type` decision and `bias_correction_cox_snell.md` (overlapping machinery).
- [ ] TODO-3: Phase 2 — Easy families: ordinary logit, probit, Poisson, modified-Poisson wrappers, Gaussian OLS one-likelihood path; optionally log-binomial/identity-binomial next.
- [ ] TODO-4: Phase 3 — record the inference policy (corrected estimate + corrected Wald; ordinary score/gradient/LR unchanged) before extending further.

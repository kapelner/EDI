...
> **Depends on:** `fix_inference_hierarchy.md` `LikelihoodTests` capability machinery (already available); gated decision. (Global ordering: see `_master.md`.)
...
| `InferenceSurvivalGLMMWeibullFrailtyNormalOneLik` | `fast_weibull_frailty_cpp` | **Difficult** | Frailty integration moves this out of the easy modified-profile regime. |
| `InferenceSurvivalGLMMWeibullFrailtyLoggammaOneLik` | `fast_clayton_weibull_aft_optim_cpp` | **Difficult** | Copula dependence plus parametric margins is too bespoke for a shared rollout. |
...
- [ ] TODO-2: If pursued: Phase 1 — register a `lik_ratio_mod_profile` testing type through the `LikelihoodTests` component with a `modified_profile_likelihood` capability (gated on `likelihood_tier` + a `get_modified_profile_loglik()`/`get_modified_profile_components()` private hook), reusing the LR memoization/null-fit warm-start machinery.
- [ ] TODO-3: Phase 2 — first families: negative binomial, beta regression, Weibull, one or two ordinal fixed-effects models.
- [ ] TODO-4: Phase 3 — extend selectively to GLMM families (Gaussian LMM, logistic/Poisson GLMM) only if Phase 2 proves valuable.

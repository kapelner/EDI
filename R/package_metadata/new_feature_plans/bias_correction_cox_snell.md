...
> **Depends on:** `expanded_estimate_report.md` (API-shape decision); pairs with `cordeiro_mccullagh_bias_correction_report.md` (same Easy-tier machinery); `fix_inference_hierarchy.md` component registration. (Global ordering: see `_master.md`.)
...
| Clayton copula Weibull AFT | `InferenceSurvivalGLMMWeibullFrailtyLoggammaOneLik` | `fast_survival_models_optim.cpp` | Copula-coupled survival margins | **Very Hard** | Copula dependence parameter adds another nuisance block with no standard closed form | research-scale, no confident estimate |
...
| Clogit+GLMM hybrid | `InferenceIncidKKCondLogitGLMMOneLik` | `fast_clogit_plus_glmm.cpp` | Hybrid partial-likelihood + GLMM | **Very Hard** | Compounds both the partial-likelihood and GLMM issues above | research problem |
| Weibull frailty | `InferenceSurvivalGLMMWeibullFrailtyNormalOneLik` | `fast_weibull_frailty.cpp` | Frailty (random-effect survival), integrated likelihood | **Very Hard** | Same marginal-likelihood issue as GLMM | research problem |
...
- The `InferenceMixinBartlettApprox`, `InferenceMixinCordeiroFerrariApprox`,
  ...
  corrections, not point-estimate bias). (Updated 2026-08-14: the Pattern-1
  mixin convention those stubs established — `list(public=..., private=...)`
  spliced via `utils::modifyList` — is retired by the completed shallow-hierarchy
  migration, which bans raw splicing. A new Cox-Snell correction lands as a
  registered `InferenceComponent` (e.g. `CoxSnellBiasCorrection`) with a
  declared capability, composed via `define_inference_class()`, mirroring the
  `BartlettApproximation` component precedent.)
...
- [ ] TODO-2: Build the shared `X'WX`-based Cox-Snell helper on top of `get_information_matrix()` (the `InformationMatrix` component's accessor) and register a `CoxSnellBiasCorrection` `InferenceComponent` with a declared capability, per the `BartlettApproximation` precedent.
- [ ] TODO-3: Roll out to the canonical-link GLM families first (logistic, probit, Poisson + its wrappers, OLS), then log-binomial/identity-binomial once the shared mu-near-boundary guard exists, then negative binomial and Cox PH (analytic information already in their kernels; only the third-derivative cumulant term is new).
- [ ] TODO-4: Coordinate the public API with `expanded_estimate_report.md`'s `estimate_type` decision (`"cox_snell"` is one of its proposed values) — decide that first so this doesn't ship as another ad hoc `compute_*_estimate()` method.
- [ ] TODO-5: Keep GLMM/frailty/copula and GEE paths explicitly out of scope, as this report recommends.

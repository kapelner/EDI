...
> **Depends on:** approx shipped. Exact rollout: gated decision; shares the GLM cumulant helper with `score_correction_cordeiro_ferrari.md`/`gradient_correction_lemonte.md` — order the three together. (Global ordering: see `_master.md`.)
...
> interval-censored-survival plans). `InferenceAsympLik`'s likelihood plumbing now lives in the `LikelihoodTests` component (gated by `likelihood_tier` metadata), `InferenceParamBootstrap` is the `ParametricLikelihoodBootstrap` component, `InferenceAsympLikStdModCache` is the `StandardModelCache` component, and `supports_*()` hook methods are capability reads (`obj$supports(...)`). Optional behavior is composed as registered `InferenceComponent`s via `define_inference_class()` — the deleted inheritance ladder's diagrams and "lives in class X" statements below should be read as "provided by the matching component". Additionally, this report's proposal has since been
> **implemented and shipped**: `lik_ratio_bartlett_approx`/`lik_ratio_bartlett_exact`
> exist as testing types, and the shared machinery lives in the
> `LikelihoodTests`/`ParametricLikelihoodBootstrap`/`BartlettApproximation`
> components. Kept as the historical feasibility analysis; the per-family
> difficulty tiers remain the reference for extending exact Bartlett support.
...
- [ ] TODO-1: Make a decision about whether to extend **exact** (analytic) Bartlett beyond `InferenceContinKKOLSOneLik` — ask the user. The approx path is done package-wide; the report's Phase-2 exact targets (logit, probit, Poisson, modified-Poisson, log-binomial, identity-binomial) each need a Cordeiro-style derivation, and the sibling score/gradient correction plans (`score_correction_cordeiro_ferrari.md`, `gradient_correction_lemonte.md`) would share the cumulant machinery — decide the ordering across all three together.
- [ ] TODO-2: If extended: follow the report's Phase 2 tier first, Phase 3 (ordinal, Cox, negative binomial, Weibull) only after, and reuse/extract a shared GLM cumulant helper with the score/gradient plans rather than deriving three times.

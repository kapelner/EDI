...
> **Depends on:** gated decision; `fix_inference_hierarchy.md`'s KK/GEE component extraction (Stage 1 extracts the clustered-fit core from the `KKGEE` component source). (Global ordering: see `_master.md`.)
...
> interval-censored-survival plans). Read its reuse-target citations through the new component names:
> `InferenceMixinKKGEEShared` is the `KKGEE` component (same source file),
> `InferenceAsympLikStdModCache` is the `StandardModelCache` component, and
> composing them means `components = c(...)` in `define_inference_class()`,
> not inheriting bases. The feasibility verdict is unchanged.
...
- [ ] TODO-2: Stage 1 — extract a response-type-agnostic clustered-fit core from the `KKGEE` component's fit code (`inference_mixin_kk_gee_shared.R`).
- [ ] TODO-3: Stage 2 — add a ragged/long-format response container to `Design` (this contradicts the one-row-per-subject storage every current path assumes — the report's "hard" verdict lives here).
- [ ] TODO-4: Stage 3 — add `InferenceLongitudinalGEE` via `define_inference_class()` (components: `Wald` + the extracted clustered core; register response-type metadata).
- [ ] TODO-5: Resolve the estimand question (marginal vs. subject-specific, which timepoint contrast) with the user before Stage 2.

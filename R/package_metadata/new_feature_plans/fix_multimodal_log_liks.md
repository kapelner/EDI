...
starts, in the same spirit as `cold_starts.md` / `multistart_nonconcave_likelihoods.md`
...
3. A reproducible random layer (seeded off the fit's own data, not global
   ...
   `multistart_nonconcave_likelihoods.md`'s pattern if that plan has already
...
- **Cost**: multi-start multiplies the per-fit cost of a class already
  ...
  (same table format as `cold_starts.md`) to size the impact on
...
- [ ] TODO-2: Design the multi-start candidate set for `generate_mod()`
  ...
  above); align with `multistart_nonconcave_likelihoods.md`'s scheme if it
...
- [ ] TODO-6: Benchmark per-fit cost before/after (table format per
  `cold_starts.md`); if the slowdown is material, gate multi-start behind a

...
> **Depends on:** `expanded_estimate_report.md` (API shape); `firth_penalties_report.md` (the report recommends Firth first — decide Firth before this). (Global ordering: see `_master.md`.)
...
- [ ] TODO-2: (The report's own Phase 1 *is* the decision gate above — it explicitly recommends Firth first if the goal is mean-bias reduction / separation resistance.)
- [ ] TODO-3: If pursued: Phase 2 — explicit `compute_median_bias_corrected_estimate()` / `_wald_*()` API, same never-redefine-the-default rule as the other correction reports.
- [ ] TODO-4: Phase 3 — start with Gaussian OLS one-likelihood, then perhaps logit/probit/Poisson; nothing harder without a fresh decision.

...
> **Depends on:** gated decision; needs its own `Design` vector-response storage project (no current dependency satisfies it). (Global ordering: see `_master.md`.)
...
> names: `InferenceMixinKKGeeShared` is the `KKGEE` component (same source file),
> and `InferenceAsympLikStdModCache` is the `StandardModelCache` component —
> "directly reusable" claims about them hold, but reuse means listing the
> component in `components = c(...)` via `define_inference_class()`, not
...
- [ ] TODO-2: Stage 1 — admit the `compositional` type and vector-per-subject storage safely in `Design` (registered via `define_design_class`-world metadata; fence every scalar-response path listed in "Package Areas That Need Explicit `compositional` Fences").
- [ ] TODO-3: Stage 2 — build the ILR transform layer.
- [ ] TODO-4: Remaining stages per the report's staged plan (first-wave inference set from "Recommended First-Wave Compositional Inference Set", `SimulationFramework` support), each new class via `define_inference_class()` with components/capabilities.
- [ ] TODO-5: Resolve the estimand question ("What Is The Compositional Estimand?") with the user before Stage 2.

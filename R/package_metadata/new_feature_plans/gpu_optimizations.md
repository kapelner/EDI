...
> **Depends on:** gated decision; `sexp_removal_rcppeigen_conversion_spec.md` (the `EDI_CORE_ONLY` Rcpp-free cores are what a GPU backend can compile against). (Global ordering: see `_master.md`.)
...
- [ ] TODO-2: Prototype GPU `compute_wilcox_distr_parallel_cpp()` for the `delta == 0` fast path (establishes build/dispatch/device-memory/test infrastructure).
- [ ] TODO-3: Prototype GPU `compute_pair_distance_matrix_cpp()`; benchmark against the raw-pointer CPU version.
- [ ] TODO-4: Prototype GPU OLS randomization/bootstrap distributions (`compute_ols_distr_parallel_cpp()` first).
- [ ] TODO-5: Reassess GLMM GPU work only after the first three prototypes; treat D/A-optimal search and ZINB/ZAP Hessians as second-wave experiments.
- [ ] TODO-6: Run the report's benchmarking matrix (CPU single-thread vs. OpenMP vs. GPU, transfer-included and -excluded, three workload sizes, parity + determinism checks) for every prototype before any is merged.
- [ ] TODO-7: Decide the backend/build story first (CUDA vs. OpenCL vs. vendor-neutral; optional dependency handling for CRAN and the `edi_kernels` wheel) — the report's "Suggested GPU Backend Design" section — since it constrains all prototypes.

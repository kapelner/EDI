...
Release line: **v1.1.0** — `../future_release_plans/release_v1_1_0.md → TODO-4b` (added 2026-08-23, user decision; `_master.md` Phase 4, second lane). The §8 TODOs (132–179) are ticked **here**, in this owning plan; the release file is only the index. Result-changing items (TODO-137, 149, 153, 156) are gated by that release's additive/bit-for-bit rule — opt-in, or an explicitly documented default change with equivalence-test tolerances re-justified.

File history: lived as `package_metadata/audits/perf_experiments.md` (git: `perf_experiments_final.md`) through 2026-08-22, then moved to `package_metadata/new_feature_plans/performance_profiling_and_upgrades.md` when the Phase 8 forward-looking audit plan (§8) was added.
...
**TODO-129: fast_trigamma — A&S 6.4.12 asymptotic expansion + recurrence shift, replacing `R::trigamma`** ✓ DONE

Files: `EDI/src/_helper_functions.h`, `EDI/src/kk21_weights.cpp`, `EDI/src/fast_hurdle_negbin.cpp`, `EDI/src/fast_negbin_regression.cpp`, `EDI/src/fast_beta_regression.cpp`, `EDI/src/fast_zero_one_inflated_beta.cpp`

Context: `fast_digamma`/`fast_lgamma`/`fast_erfc` already replace their `R::` equivalents at most hot call sites (see TODO-52, TODO-71, TODO-77, TODO-78), but `R::trigamma` — psi', used in every negbin and beta-regression Fisher information/Hessian path — had no fast replacement and was still called at 20 sites, all inside per-observation loops.

**Implemented (2026-07-28):**
- `fast_trigamma(double x)`: same shape as `fast_digamma` — shift `x` up via the recurrence `psi'(x) = psi'(x+1) + 1/x^2` until `x >= 8`, then evaluate the A&S 6.4.12 asymptotic series (Horner form) at the shifted argument. Falls back to `R::trigamma` for `x <= 0`.
- `fast_trigamma_vec(const Eigen::ArrayXd&)`: batched version for the loop call sites. Applies the recurrence shift with a per-lane mask (`active_d = (xx < 8.0).cast<double>()`) instead of a per-lane branch, for a fixed number of full-width Eigen array ops (`n_shifts = ceil(8 - min(x))`) rather than `n` independent scalar loops. First version reallocated `active`/`active_d`/`inv` on every shift iteration; `perf report` on a realistic-argument-range benchmark showed `_int_malloc`/`_int_free_merge_chunk`/`memset` eating a measurable share of cycles inside the function — fixed by hoisting a single reusable `active_d` buffer outside the shift loop. Note this batched design has a real tradeoff, not just a win: cost scales with the *smallest* element in the batch, so a batch containing a near-zero outlier pays the same full-width shift cost as every other lane in that batch. It's a net win for the actual call sites here (`y + theta`, `mu * phi`, `(1-mu) * phi` — all bounded away from zero in practice) but would not necessarily win on an adversarially wide-range batch.
- Applied at all 20 call sites: scalar swaps for one-off `trigamma(theta)`/`trigamma(phi)` calls; `fast_trigamma_vec` for the per-observation-loop sites (`kk21_weights.cpp`'s two `info +=` Newton-Raphson loops, `fast_hurdle_negbin.cpp`/`fast_negbin_regression.cpp`'s distinct-`y` table-fill loops, `fast_beta_regression.cpp`'s `TrigammaFunctor`-via-`unaryExpr` calls replaced outright with `fast_trigamma_vec(a.array())`/`fast_trigamma_vec(b.array())`, `fast_zero_one_inflated_beta.cpp`'s two per-observation Hessian loops). `fast_zero_one_inflated_beta.cpp`'s second loop also had an invariant `R::trigamma(phi)` recomputed on every iteration; hoisted it outside the loop as part of the same change.

Correctness: `fast_trigamma`/`fast_trigamma_vec` verified against both Boost.Math (`boost::math::trigamma`, via `BH`) and `R::trigamma` across a dense grid spanning `[1e-6, 1e4]` — max relative error 3.23e-12 against both references (`benchmark/fast_trigamma_correctness.R`, not part of the installed package's test suite since it's a one-off cross-reference check, not a regression test — see `[[feedback_internal_functions_edi_triple_colon]]`). `fast_trigamma_vec` matches `fast_trigamma` element-wise to machine precision. Existing regression suites that exercise every migrated call site (`test-kk21-negbin-fast-digamma.R`, `test-beta-regression-preallocated-eta-mu.R`, `test-hurdle-negbin-log1p-fast-path.R`, `test-argument-permutations.R`, `test-custom-implementation-canonical-reductions.R`) pass unchanged after the swap. Installed with `R CMD INSTALL --no-docs`.

Profiling: `perf record -F 999` + `perf report --sort symbol` on a `bench_fast_trigamma_scalar_loop_cpp` harness (`benchmark/fast_trigamma_profile.cpp`, cached via `sourceCpp(cacheDir=...)` so the profile isn't dominated by compilation) shows 87% of cycles inside the function itself once R/package-load overhead is amortized over enough reps, confirming the cost is genuinely the arithmetic, not R-boundary overhead. `perf stat` IPC 1.40, branch-misses 0.8-1.5% of branches — no evidence of a branch-prediction or memory-stall bottleneck in the scalar path.

Benchmark vs `R::trigamma` (`benchmark/fast_trigamma_speed_compare.R`): paired ABBA/BAAB-ordered timing, `N = 500` arguments drawn as `y + theta`-shaped (`rpois(8) + runif(2,12)`, realistic call-site range), 50 reps per timed call, 30 rounds, single-thread wall time via `std::chrono::steady_clock`. `fast_trigamma` median 0.159ms (IQR 0.151-0.171ms) vs `R::trigamma` median 4.711ms (IQR 4.464-5.871ms); median paired speedup 29.6x (bootstrap 95% CI 28.7x-31.3x); paired one-sided Wilcoxon p = 9.31e-10.

---

**TODO-130: fast_qnorm / fast_lbeta / fast_dnbinom_mu — last four `R::` call sites in the model-fitting core, replacing `R::qnorm5`/`R::pnorm5`/`R::lbeta`/`R::dnbinom_mu`** ✓ DONE

Files: `EDI/src/fast_erfc.h`, `EDI/src/fast_gamma_functions.h`, `EDI/src/fast_probit_regression.cpp`, `EDI/src/fast_ordinal_probit_regression.cpp`, `EDI/src/fast_hurdle_negbin.cpp`, `EDI/src/beta_regression_helpers.cpp`

Context: the 2026-07-28 R-dependency audit of the 33-file model-fitting core (`package_metadata/python_bindings_package_spec.md`) found exactly 4 real (non-comment) `R::` call sites left outside the already-migrated `fast_digamma`/`fast_lgamma`/`fast_trigamma`/`fast_erfc` family: `R::pnorm5` x2 and `R::qnorm5` x1 in `fast_probit_regression.cpp`, `R::qnorm5` x1 in `fast_ordinal_probit_regression.cpp`, `R::dnbinom_mu` x1 in `fast_hurdle_negbin.cpp` (2 other grep hits in this file and `fast_negbin_regression.cpp` are comments, not calls), and `R::lbeta` x1 in `beta_regression_helpers.cpp`.

**Implemented (2026-07-29):**
- **`fast_log_pnorm`** already existed in `fast_erfc.h` (log-scale standard normal CDF via the existing Cephes `fast_erfc` piecewise rational approximation, clamped outside `|x| >= 8`) — no new code needed, just swapped both `fast_probit_regression.cpp` `R::pnorm5(x,0,1,{1,0},1)` call sites (`log_pnorm_lower`/`log_pnorm_upper`'s `|x| >= 6` fallback branch) to `fast_log_pnorm(x)`/`fast_log_pnorm(-x)`.
- **`fast_qnorm(double p)`** (new, `fast_erfc.h`): standard normal quantile function via Peter Acklam's rational approximation (~1.2e-9 relative error across the full `(0,1)` range including extreme tails). A Halley refinement step (using the existing `pnorm_fast`/`dnorm_fast`) was implemented and correctness-verified first, then **removed** after the speed benchmark showed it made `fast_qnorm` *slower* than `R::qnorm5` (0.462x, i.e. a regression) — `R::qnorm5`'s own AS-241-based implementation is already near-optimal for this task, and the refinement's extra `fast_erfc`/`exp` calls cost more than they returned. Both real call sites (`fast_probit_regression.cpp`'s and `fast_ordinal_probit_regression.cpp`'s cold-start/warm-start heuristics, both further refined by the optimizer regardless) have no need for better than ~1e-9, so the unrefined ~2x-faster approximation was kept instead.
- **`fast_lbeta(double a, double b)`** (new, `fast_gamma_functions.h`): `fast_lgamma(a) + fast_lgamma(b) - fast_lgamma(a+b)`, composed directly from the already-migrated `fast_lgamma` — trivial once `fast_lgamma` existed.
- **`fast_dnbinom_mu(double x, double size, double mu, bool return_log)`** (new, `fast_gamma_functions.h`): explicit negative-binomial log-PMF via `fast_lgamma`, mirroring the identical explicit-log-PMF formula already used in this same file's hot neg-log-likelihood loop (see the `// Explicit NB log-PMF — replaces R::dnbinom_mu overhead` comment a few lines above the one real call site) — just packaged as a reusable function for the one remaining direct-call site (a per-`(y,i)` truncation-probability loop). Parameter/local names avoid `give_log`/`log_p`: Rcpp's `Rcpp/stats/dpq/macros.h` `#define`s `give_log` to `log_p` at include time, which silently rewrote an initial version of this signature and collided with a same-named local — caught at compile time (`declaration of 'const double log_p' shadows a parameter`), fixed by naming the parameter `return_log` and the local `logp_val`.

Correctness: `benchmark/fast_qnorm_lbeta_dnbinom_correctness.cpp/R` cross-references all four against Boost.Math (`boost::math::quantile(normal())`, `boost::math::lgamma`-composed log-beta, `boost::math::cdf`) and `R::` directly. `fast_qnorm`: max relative error 1.13e-9 (both references) across `p` spanning `1e-12` to `1-1e-12`. `fast_log_pnorm`: max absolute error 1.42e-14 across its validated `|x| <= 8` domain (matching `test-fast-probit-cdf.R`'s own existing `7.9` grid boundary — testing beyond `|x| = 8` isn't a fair check of this pre-existing, unchanged function, since it's a deliberately clamped approximation there by design, not a defect: the exact `x = -8` clamp constant itself carries a small ~0.036 pre-existing absolute imprecision unrelated to this work). `fast_lbeta`: max relative error 2.95e-10 against a `boost::math::lgamma`-composed log-beta reference (a straight `log(boost::math::beta(a,b))` reference was tried first but rejected — it underflows/loses precision for large `(a,b)` via its non-log ratio-of-gammas computation, giving a false-positive "error" that was actually the reference's own imprecision, not `fast_lbeta`'s). `fast_dnbinom_mu`: max relative error 6.04e-14 (probability scale) / 2.29e-14 (log scale) against `R::dnbinom_mu` across realistic `(x, size, mu)` ranges.

Benchmark (`benchmark/fast_qnorm_lbeta_dnbinom_speed_compare.cpp/R`): same paired ABBA/BAAB-ordered methodology as TODO-129, `N = 500`, 50 reps per timed call, 30 rounds. `fast_qnorm` median 0.227ms vs `R::qnorm5` median 0.546ms — 2.37x (bootstrap 95% CI 2.31x-2.42x, Wilcoxon p = 9.31e-10). `fast_log_pnorm` median 0.852ms vs `R::pnorm5(log=TRUE)` median 2.510ms — 2.93x (CI 2.85x-3.15x, p = 5.96e-7). `fast_lbeta` median 2.100ms vs `R::lbeta` median 4.555ms — 2.17x (CI 2.07x-2.23x, p = 9.31e-10). `fast_dnbinom_mu` median 3.564ms vs `R::dnbinom_mu` median 10.905ms — 2.67x (CI 2.56x-3.09x, p = 9.31e-10).

Net effect: zero remaining `R::` calls in any of the 33 model-fitting `.cpp` files themselves. Correction to an overclaim in an earlier draft of this entry: this does *not* mean zero `R::` calls reachable from the model-fitting core overall — `_helper_functions.h` (`R::pchisq` x3) and, at the time, `_glmm_links.h` (`R::pnorm`/`R::dnorm` x4 in `ProbitLink`) are shared headers these files transitively include and call into during actual computation, not just comments or dead code; see the 2026-07-29 follow-up below for the `_glmm_links.h` half of that gap. Full EDI test suite (837 blocks, 4293 expectations) passes unchanged after the swap; targeted subset covering all 4 call sites' consuming test files (`test-fast-probit-cdf.R`, `test-incidence-probit.R`, `test-hurdle-negbin-*.R`, `test-beta-regression-preallocated-eta-mu.R`, `test-ordinal-cauchit-*.R`, `test-ordinal-cloglog-*.R`, `test-glmm-cpp-equivalence.R`, `test-clogit-plus-glmm-cpp-equivalence.R`) — 35 test blocks, 140 expectations — passes. Installed with `R CMD INSTALL --no-docs`.

**Follow-up (2026-07-29): `_glmm_links.h`'s `ProbitLink` wired to `pnorm_fast`/`dnorm_fast`.** A direct R-dependency re-audit (prompted by a user question — "are you sure the 33 C++ paths no longer have any dependency on R?") found `ProbitLink::cdf`/`pdf`/`pdf_from_cdf`/`deriv_pdf` in `_glmm_links.h` still called `R::pnorm(x,0,1,1,0)`/`R::dnorm(x,0,1,0)` directly, reachable from any GLMM kernel templated on a probit link (currently `fast_ordinal_clmm.cpp`'s `link = "probit"` path). This was a pure wiring gap, not a missing implementation: `fast_erfc.h`'s `pnorm_fast`/`dnorm_fast` already existed (built alongside `fast_qnorm` above) and were already validated to machine precision against Boost.Math and `R::pnorm`/`R::dnorm`. Swapped all four `ProbitLink` methods to call `pnorm_fast`/`dnorm_fast` instead — no new code, no new correctness/speed harness needed (the functions being called were already proven). Verified: `EDI:::fast_ordinal_clmm_cpp(..., link = "probit")` fits and converges correctly on synthetic data; `glmm-cpp-equivalence`/`fast-probit-cdf`/`clogit-plus-glmm-cpp-equivalence` (27 expectations) and the full test suite (837 blocks) pass unchanged. Installed via the touch-then-rebuild pattern (only `fast_ordinal_clmm.cpp`, the sole `.cpp` file using `ProbitLink`, needed recompiling; this `Makevars` has no header-dependency tracking, so editing `_glmm_links.h` alone does not trigger a rebuild of its includers — confirmed via `R CMD INSTALL --no-docs` producing `make: Nothing to be done for 'all'` until the dependent `.cpp` file was touched).

Remaining Rmath footprint in the model-fitting core after this follow-up: just **`R::pchisq`** (3 sites, `_helper_functions.h`'s shared LRT/score-test p-value helpers) — down from 15 at the start of the audit, 7 after the initial TODO-130 work, now 3. No `fast_pchisq` exists yet; unlike everything eliminated so far, this would be genuinely new implementation work (chi-squared CDF via the regularized incomplete gamma function) rather than a composition of existing `fast_lgamma`/`fast_erfc` machinery or a pure wiring fix. See `package_metadata/python_bindings_package_spec.md`'s `Standalone Rmath Library Dependency` section for the current residual and the standalone-`libRmath`-vs-vendor tradeoff.

**Follow-up (2026-07-29): `fast_pchisq_upper` — the last remaining `R::` dependency, genuinely new implementation.** Unlike `pnorm`/`dnorm`/`qnorm`/`lbeta`/`dnbinom_mu` above (each either already vendored or a cheap composition of existing `fast_lgamma`), no chi-squared CDF existed anywhere in the codebase. Implemented via the standard regularized-incomplete-gamma relation `P(X > x) = Q(df/2, x/2)` for `X ~ chi-squared(df)` — the only `(lower_tail, log_p)` combination any of the 3 real call sites use is `(FALSE, FALSE)`, i.e. `R::pchisq(statistic, df, false, false)`, so only `fast_pchisq_upper(statistic, df)` was implemented (matching the `fast_qnorm`/`fast_dnbinom_mu` precedent of implementing only the actually-used signature, not R's full parameter surface).

**Implemented (`fast_gamma_functions.h`):**
- `fast_gammap_series(a, x)`: regularized lower incomplete gamma `P(a,x)` via series expansion (A&S 6.5.29), convergent for `x < a+1`.
- `fast_gammaq_cf(a, x)`: regularized upper incomplete gamma `Q(a,x)` via a continued fraction, modified Lentz's method (A&S 6.5.31), convergent for `x >= a+1`.
- `fast_gammaq(a, x)`: dispatches between the two above (`x < a+1` -> `1 - fast_gammap_series`; `x >= a+1` -> `fast_gammaq_cf`), each stopping at a `1e-16` relative tolerance on successive terms (capped at 500 iterations, never hit in testing). Both use `fast_lgamma` for their `exp(-x + a*log(x) - lgamma(a))` leading factor, so accuracy is governed by `fast_lgamma`'s already-validated precision plus the small truncated-series/CF residual.
- `fast_pchisq_upper(statistic, df) = fast_gammaq(df/2, statistic/2)`.
- Naming note: parameter/local names in `fast_dnbinom_mu` already had to dodge Rcpp's `give_log`->`log_p` macro; `fast_pchisq_upper` has no such collision since it takes no boolean flag at all (only the one needed signature was implemented).

**Wired into all 3 real call sites**, all in `_helper_functions.h`: `likelihood_ratio_test_from_negloglik` (general integer `df`, from the `df` parameter), `score_test_from_score_information` (`df=1.0` fixed), `gradient_test_from_restricted_score` (`df=1.0` fixed).

Correctness (`benchmark/fast_pchisq_correctness.cpp/R`): cross-referenced against `boost::math::cdf(boost::math::complement(chi_squared_distribution(df), statistic))` and `R::pchisq(statistic, df, false, false)` directly, across statistic spanning `1e-6` to `1e4` (log-spaced, covering both central and extreme-tail p-values) and df spanning realistic small integers (1-10, matching the real call sites) plus a wider general range (0.5-100). Max relative error: 9.2e-14 vs Boost.Math, 1.31e-13 vs `R::pchisq` — both far under the 1e-9 threshold. Explicit check at the 4 real call-site df values (1, 2, 3, 5): max relative error 3.49e-14. End-to-end verification through the actual exported wrappers (`likelihood_ratio_test_from_negloglik_cpp`, `score_test_from_score_information_cpp`, `gradient_test_from_restricted_score_cpp`, not just the standalone harness) against `stats::pchisq()` called directly in R: all three agree to ~1e-15 relative error (machine precision) on synthetic score/information/log-likelihood inputs.

Benchmark (`benchmark/fast_pchisq_speed_compare.cpp/R`): same paired ABBA/BAAB-ordered methodology as TODO-129/TODO-130, `N = 500` statistics drawn as `rchisq(df)` with `df` sampled from `1:5` (matching the real call-site distribution), 50 reps per timed call, 30 rounds. `fast_pchisq_upper` median 2.399ms vs `R::pchisq` median 5.031ms — 2.10x (bootstrap 95% CI 2.03x-2.24x, Wilcoxon p = 9.31e-10).

**Net effect: this eliminates the last remaining `R::` call in the entire 33-file model-fitting core.** Zero `R::` calls remain anywhere in scope — direct or via a transitively-included shared header — except the intentional, unreachable-in-normal-use `x <= 0` fallback inside `fast_digamma`/`fast_trigamma` themselves. The Rmath-dependency count for this scope has gone 15 -> 7 -> 3 -> 0 across this session's TODO-130 work and its two follow-ups. Full EDI test suite (837 blocks, 4293 expectations) passes unchanged after the swap.

Installation note: the `R CMD INSTALL --no-docs` for this change (50 dependent `.cpp` files touched, since `_helper_functions.h` is the most widely-included shared header in this scope and this `Makevars` has no header-dependency tracking) got interrupted mid-install — the build itself completed successfully (all 50 files compiled, `EDI.so` linked) and was fully staged in `00LOCK-EDI/00new/EDI/`, but the process was killed before R's final move-into-place step, leaving the live `EDI/` directory empty and an orphaned `00LOCK-EDI`. Confirmed via `ps aux` that no install/compiler process was still running, then manually completed the interrupted install (moved the fully-built staged package into place, removed the now-orphaned lock) rather than re-running the whole 50-file rebuild from scratch.

---

## Phase 7 — Kernel Roster Freshness Audit (2026-08-08)

This document was last updated 2026-07-29 (TODO-130 and its two follow-ups). Since then, 9 commits have touched `EDI/src/` — python-bindings work, a class-hierarchy overhaul, a log-binomial regression fix, and general cleanup (`git log --oneline --since=2026-07-29 -- EDI/src/`). This pass re-verifies TODO-130's headline claim and catalogs what's new since the last full kernel sweep (Phase 6, 2026-07-02, ~129 kernels).

**TODO-130's "zero `R::` calls in the model-fitting core" claim: re-confirmed, still holds.** `grep -rn "R::" EDI/src/*.cpp EDI/src/*.h` (excluding comments) currently finds 31 hits, all outside the 33-file model-fitting-core scope that claim was actually about: `R::unif_rand`/`R::rbeta`/`R::rpois`/`R::rweibull`/`R::runif` in randomization/resampling files (`pocock_simon_assign.cpp`, `atkinson_assign.cpp`, `atkinson_redraw_batch.cpp`, `kk14_redraw.cpp`, `efron_redraw.cpp`, `simulation_dgp.cpp`, `bootstrap_indices.cpp`, `bootstrap_match_indices.cpp`, `exchangeable_resampling_draws.cpp`, `fast_sample_int.cpp`, `generate_permutations.cpp`, `binary_match_search.cpp` — RNG draws, not model-fit likelihood/score/Hessian computation), `R::dchisq` in `lrt_ci_newton.cpp`, `R::dbinom` in `zhang_exact_speedups.cpp`, and the intentional `x <= 0` fallback branches inside `fast_gamma_functions.h`'s `fast_digamma`/`fast_trigamma` that TODO-129 already documented as unreachable in normal use. No regression.

**Kernel roster has grown well past the last full-sweep count.** `grep -A2 "^// \[\[Rcpp::export\]\]" EDI/src/*.cpp` currently finds **332** exported kernels, vs. the ~129 counted as of the 2026-07-04 sweep referenced elsewhere in this document. Diffing every current export against every kernel/function name mentioned anywhere in this document finds **258** with no prior mention. Breaking that 258 down by pattern (not all need individual profiling — see recommendation below):

- **21 kernels**: a systematic `*_weighted_cpp` / `get_*_weighted_score_cpp` / `get_*_weighted_hessian_cpp` family (e.g. `fast_logistic_regression_weighted_cpp`, `fast_poisson_regression_weighted_cpp`, `get_hurdle_poisson_glmm_weighted_score_cpp`) — this lines up with the Bayesian-bootstrap weighted-observation effort tracked in `package_metadata/WEIGHTED_OBSERVATION_IMPLEMENTATION_CHECKLIST.md`. These are real per-observation-loop kernels on likely-hot bootstrap paths and have never been profiled.
- **67 kernels**: `get_*_score_cpp` / `get_*_hessian_cpp` pairs across most response families (logistic, Poisson, negbin, beta, ordinal cauchit/cloglog/probit, hurdle negbin/Poisson GLMM, Weibull/Weibull-frailty, CoxPH/stratified CoxPH, adjacent-category logit, continuation-ratio, dep-cens-transform, clogit+GLMM, cpoisson-combined, Clayton-Weibull-AFT, zero-one-inflated beta, gaussian LMM, stereotype logit) — these back the `get_likelihood_test_spec()` Wald/score/LR/gradient-test surface. Also never profiled as a family; likely dominates any future sweep's real findings given how many response families now share this pattern.
- **12 kernels**: `fast_ordinal_*_with_var_cpp` variants (cauchit/cloglog/probit regression, `_weighted` too) — variance-path siblings of already-profiled estimate-only ordinal kernels.
- **~5 kernels**: `test_*_cpp` exports (`test_ols_smart_cold_start_beta_cpp`, `test_ordinal_start_cpp`, `test_weibull_aft_start_cpp`, `test_finalize_warm_start_beta_cpp`) — internal testthat scaffolding for cold/warm-start unit tests, not perf-sensitive production kernels; excluded from the profiling recommendation below.
- **remainder (~150)**: small O(n) utility/accessor kernels (`mean_cpp`, `var_cpp`, `matrix_rank_cpp`, `shuffle_cpp`, `get_column_types_cpp`, `count_unique_values_cpp`, result-key-store cache management, distance-matrix builders, permutation generators, etc.) — plausible candidates for the package's general "Utility / Math Kernel Performance" treatment (see `benchmark/benchmark_model_fits.R`) rather than dedicated per-kernel profiling entries; not itemized individually here.

No timing numbers are fabricated for any of the above — none of it has been profiled yet under this document's methodology (paired ABBA/BAAB `perf`+`std::chrono` benchmarking against a real reference implementation), and inventing numbers would contradict this document's own standard of evidence.

- [ ] TODO-131: Run a Phase-6-style parallel kernel sweep scoped to the two families most likely to matter — the 21 `*_weighted_*` kernels and the 67 `get_*_score_cpp`/`get_*_hessian_cpp` pairs (88 kernels total) — profiling each under realistic `n`/`p` per the existing benchmark harness conventions (`benchmark/comprehensive_benchmark.R`, `benchmark/benchmark_inference_all.R`), and append real TODO-132+ entries here only for kernels that show a genuine bottleneck, following this document's established "root cause → fix → correctness → paired-benchmark" format. Do not profile the `test_*_cpp` scaffolding or the ~150 trivial utility kernels as part of this sweep.

---

## Phase 8 — Further Performance-Audit TODOs (2026-08-22)

Everything above profiles one kernel at a time with `perf record --no-children` (self time), `perf stat` (IPC / branch-misses), and `perf annotate`, then fixes what the top symbols point at: special functions, allocator churn, rank-1 → GEMV, exp-count reduction. That has been productive, but it also means (a) several whole *dimensions* of performance have never been audited and (b) only two tools — `perf` and, once, `valgrind` — have ever been pointed at the code. The TODOs below are organized by dimension and each names the specific tool(s) to use. Each is phrased as an *audit* (measure first, following this document's "root cause → fix → correctness → paired-benchmark" standard); only the measurements decide whether a fix entry gets written. Numbering continues from TODO-131; the kernel-sweep findings that TODO-131 promises as "TODO-132+" should instead take the next free number after this section.

Ground truth as of this writing (checked against the tree, not recalled from memory): 108 `.cpp` files, 338 `Rcpp::export`s, 86 files containing OpenMP pragmas (28 `schedule(static`, 12 `schedule(dynamic`), 1060 `Eigen::Map` uses vs 33 `as<Eigen::MatrixXd/VectorXd>` deep copies, 75 `.row(` call sites, 16 `checkUserInterrupt` sites, ~93k lines of R, 288 testthat files (~38k lines). `configure` defaults: `EDI_PORTABLE=0` (`-march=native -mtune=native`), `EDI_DISABLE_VECTORIZATION=0` (so `EIGEN_DONT_VECTORIZE` is **no longer** set by default — see TODO-136), `EDI_NATIVE_LTO=0`, `EDI_DEBUG_SYMBOLS=0` (`-g0`), `EDI_UNITY=1`, `-O3`.

### 8.0 Tool roster — what exists on Ubuntu and what each is for

The point of this table is that every later TODO can name its tool, and that no class of tool is left unused. Install column: `apt` = Ubuntu package, `pip` = PyPI, `src` = build from GitHub, `oneAPI` = Intel's apt repo (`intel-oneapi-vtune`, `intel-oneapi-advisor`), `CRAN` = R package.

| Class | Tool | Install | What it gives us that nothing else here does |
|---|---|---|---|
| **Sampling / HW-counter profilers** | `perf` — `record -g --call-graph dwarf`, `report --children --sort srcline`, `annotate -l`, `stat --topdown -r N`, `diff` (before/after of two `perf.data`), `c2c` (false sharing), `mem` (load/store latency), `sched` (thread wake/sleep), `probe` (dynamic uprobes), `trace`, `bench`, `record -e intel_pt//u` + `script --insn-trace --xed` (exact instruction trace, no sampling) | apt `linux-tools-$(uname -r)` | the baseline; `diff`, `c2c`, `mem`, `--topdown`, and Intel PT have never been used here |
| | `pmu-tools` (`toplev.py`, `ocperf.py`) | src | proper Top-Down Microarchitecture Analysis (TMA) down to level 3+ with per-uarch event maps; cleaner than raw `perf stat --topdown` |
| | `likwid` (`likwid-perfctr -g FLOPS_DP\|MEM\|L2CACHE\|BRANCH -m` marker API, `likwid-bench`, `likwid-topology`, `likwid-pin`, `likwid-powermeter`) | apt | derived metrics (GFLOP/s, memory bandwidth GB/s, cache miss ratios) per marked region; machine ceilings for a roofline |
| | Intel VTune (`vtune -collect hotspots\|uarch-exploration\|memory-access\|threading\|hpc-performance`) | oneAPI | GUI-grade microarchitecture + OpenMP spin/wait attribution + vectorization % per loop; runs fine on `Rscript` |
| | Intel Advisor (`advisor --collect=survey\|roofline\|map\|dependencies`) | oneAPI | per-loop vectorization report with *why not* + roofline placement + a dependency analysis that proves a loop safe for `#pragma omp simd` |
| | AMD uProf (`AMDuProfCLI collect --config tbp\|ibs`) | src (AMD .deb) | the VTune equivalent on AMD hosts; IBS gives precise-event sampling |
| | gperftools CPU profiler (`LD_PRELOAD=libprofiler.so CPUPROFILE=…`) + `pprof` | apt `google-perftools` | signal-based sampling that works where `perf_event_paranoid` blocks `perf` (CI boxes, containers) |
| | `uftrace` (`-pg` or `-finstrument-functions`, or `-P.` dynamic) | apt `uftrace` | *exact* function call counts and per-call durations with a call graph (`uftrace report/graph/tui`) — evaluation counts per fit without hand instrumentation |
| | `gprof` (`-pg`) | apt `binutils` | the classic; mostly superseded by `uftrace`, listed for completeness |
| | eBPF: `bpftrace`, `bcc-tools` (`funclatency`, `funccount`, `profile`, `offcputime`, `memleak`) | apt | uprobe latency *histograms* and call counts on the installed `EDI.so` inside a real R session — no harness, no rebuild; off-CPU time for waiting/blocking |
| | `systemtap` | apt | same niche as bpftrace with scripting; optional |
| | `ltrace -c` / `strace -c -T` | apt | library-call and syscall counts/time (malloc/free totals, `openat` storms at `library(EDI)`) |
| | `magic-trace` | src | Intel PT with a UI for one exact trace of a single fit |
| | Visualizers: `hotspot` (perf GUI), `speedscope`, `FlameGraph`, `gprof2dot`, `kcachegrind`, `perfetto`/Tracy SDK (manual zones) | apt `kcachegrind`; pip `gprof2dot`; src `FlameGraph`; `hotspot` = AppImage from KDAB releases (not in apt); `speedscope` = npm or web | flame graphs and call-graph browsing for the above |
| **Instrumenting simulators / memory tools** | `valgrind --tool=callgrind --dump-instr=yes --collect-jumps=yes` | apt | deterministic per-instruction / per-line counts, noise-free A/B |
| | `valgrind --tool=cachegrind --cache-sim=yes --branch-sim=yes` | apt | simulated I1/D1/LL misses and branch mispredicts per line |
| | `valgrind --tool=dhat` | apt | allocation *sites* with lifetimes, bytes read/written per block, "never-read"/"short-lived" blocks — the tool for allocator churn, better than reading `_int_malloc` in `perf` |
| | `valgrind --tool=massif` + `ms_print`/`massif-visualizer` | apt | heap profile over time, peak and who holds it |
| | `valgrind --tool=lackey`, `--tool=exp-bbv` | apt | instruction-mix counts (scalar vs SIMD, loads vs ALU) |
| | `valgrind --tool=helgrind` / `--tool=drd` | apt | OpenMP data races (a race "fixed" with `atomic`/`critical` is a perf bug) |
| | Intel SDE (`sde64 -mix`, `-footprint`, `-dcfg`, `-hsw`/`-skl`/`-spr` emulation) | src (Intel) | exact dynamic instruction-mix histogram (scalar vs packed, FMA, AVX-512); emulate older ISAs to test `EDI_PORTABLE` without the hardware |
| | Intel Pin | src (Intel) | custom instrumentation (rarely needed once SDE/valgrind are in use) |
| | `heaptrack` + `heaptrack_gui` | apt | allocation count/size/site with flame graph; peak RSS; leak-free temporaries |
| | gperftools `tcmalloc` heap profiler, `jemalloc` (`MALLOC_CONF=prof:true` + `jeprof`) | apt `libgoogle-perftools-dev` (ships `libtcmalloc*.so` on 26.04), `libjemalloc2`/`-dev` | allocator A/B via `LD_PRELOAD` and allocator-native profiling |
| | glibc `mtrace`, `memusage`/`memusagestat`, `ltrace -e malloc+free+realloc` | apt | cheap malloc counting when valgrind is too slow |
| | `/usr/bin/time -v`, `pmap`, `smem`, `/proc/<pid>/smaps`, `numastat -p` | apt | peak RSS, mapped memory, NUMA placement |
| | `LD_DEBUG=statistics` / `LD_DEBUG=bindings` | glibc | dynamic-link relocation count/time at `library(EDI)` |
| **Assembly / instruction-level analyzers** | `llvm-mca` (`-bottleneck-analysis -timeline -iterations 100 -mcpu=native`) | apt `llvm` | static throughput/latency/port-pressure and critical path of a hot loop's asm; "is this loop dep-chain-bound or port-bound?" |
| | OSACA (`osaca --arch ZEN4\|ICX\|… file.s`) | pip `osaca` | port binding + loop-carried dependency analysis (explicitly models the LCD `llvm-mca` under-reports); supports AMD and ARM |
| | uiCA (`uiCA.py -arch SKL\|ICL\|…`) | src | cycle-accurate simulator for Intel cores; the most accurate of the three on Intel |
| | IACA | src (deprecated) | only for old uarches; superseded by uiCA |
| | nanoBench, `uops.info` | src / web | measure actual instruction latency/throughput on *this* core when the tables disagree |
| | `objdump -d -M intel --no-show-raw-insn`, `llvm-objdump`, `gdb -batch -ex 'disassemble /s'`, `perf annotate --stdio -l`, `gcc -S -fverbose-asm` | apt | get the asm; count `vfmadd*`, `ymm`/`zmm`, `call *@plt`, `vzeroupper`, `cvtsi2sd` (int→double churn) |
| | GCC opt reports: `-fopt-info-vec-optimized\|missed\|all`, `-fopt-info-inline`, `-fopt-info-loop`, `-fdump-tree-vect-details`, `-fdump-ipa-inline`, `-Winline` | gcc | *why* a loop was not vectorized/inlined, per file — never collected here |
| | Clang opt reports: `-Rpass=.*`, `-Rpass-missed=loop-vectorize`, `-Rpass-analysis=loop-vectorize`, `-fsave-optimization-record` + `opt-viewer.py`/`opt-stats.py` | apt `clang` | the same with an HTML browser; cross-checking two compilers' vectorizers is itself informative |
| | Compiler Explorer (local `godbolt` container) | src | interactive asm diffing of a single hot function under flag variants |
| | `readelf -r`, `nm -C --size-sort`, `size`, `bloaty`, `c++filt` | apt `binutils`; `bloaty` = src (not in apt) | relocation counts, biggest symbols, section sizes, binary-size diffs |
| | `pahole -C <class> EDI.so` (needs `-g`), `clang -Wpadded` | apt `dwarves` | struct/class padding and cacheline straddling of objective functors and per-thread accumulators |
| | `-fstack-usage` | gcc | per-function stack frame size (large stack arrays in hot recursive/inlined paths) |
| | `-ftime-report`, clang `-ftime-trace` + ClangBuildAnalyzer, `include-what-you-use`, `ccache -s` | apt / src | compile-time profiling (TODO-164) |
| **Static analyzers / sanitizers** | `clang-tidy -checks=performance-*,modernize-pass-by-value`, `cppcheck --enable=performance`, `gcc -fanalyzer`, `-Wall -Wextra -Wconversion -Wdouble-promotion -Wfloat-conversion` | apt | mechanical detection of copies, missing `reserve`, pass-by-value `Eigen::MatrixXd`, int↔double churn |
| | `-fsanitize=undefined` (UB that blocks optimization), `-fsanitize=address`, `-fsanitize=thread` / Archer (OpenMP) | gcc/clang | UB is both a correctness and a performance question (the optimizer assumes it away); TSan/Archer for OMP races |
| | R: `lintr`, `covr` (dead code), `R CMD check --timings`, `testthat` reporter timings | CRAN | R-side hygiene and test-time ranking |
| **Machine state & benchmark hygiene** | `lscpu`, `cpuid`, `hwloc`/`lstopo`, `numactl --hardware` | apt | topology: cores/SMT/caches/NUMA — needed to interpret every OMP result |
| | `turbostat`, `cpupower frequency-info`, `/sys/devices/system/cpu/intel_pstate/no_turbo`, `powertop`, `s-tui`, `lm-sensors`, `perf stat -e power/energy-pkg/` (RAPL) | apt | actual MHz/temperature/C-states/power *during* a benchmark — detects thermal throttling and AVX-512 frequency licensing; energy per fit |
| | `taskset`, `numactl --physcpubind --membind`, `cset shield`, `chrt`, `setarch -R` (ASLR off) | apt | pinning and determinism for small-diff A/Bs |
| | `stress-ng --cpu N --vm M` | apt | reproduce the "dev box under contention" scenario `local_machine_optimization.md` worries about, on purpose |
| | `hyperfine --warmup 3 --runs 30 --export-json` | apt | CLI-level statistical A/B (`Rscript` under different env vars / BLAS / flags) with proper warmup and outlier detection |
| | `perf stat -r 30 --table`, `perf diff` | apt | counter-level paired before/after |
| | sysstat (`mpstat -P ALL`, `pidstat -t -u -r`, `vmstat`, `sar`), `htop`/`top -H` | apt | per-thread CPU/RSS over time; idle cores during a supposedly parallel region |
| | OpenMP runtime: `OMP_DISPLAY_ENV=VERBOSE`, `OMP_DISPLAY_AFFINITY=TRUE`, `OMP_PROC_BIND`/`OMP_PLACES`, `OMP_WAIT_POLICY`, `GOMP_SPINCOUNT`, `GOMP_DEBUG=1` | libgomp | see and control thread placement and spin behavior — spinning OMP threads steal CPU from forked R workers |
| | BLAS visibility: `MKL_VERBOSE=1`, `OPENBLAS_VERBOSE=2`, `flexiblas` (`FLEXIBLAS=NETLIB\|OPENBLAS\|MKL\|BLIS`, no R rebuild), `RhpcBLASctl`, `benchmarkme`, `sessionInfo()` | `flexiblas` = src (not in apt; `update-alternatives --config libblas.so.3-x86_64-linux-gnu` between apt `libopenblas-dev`/`libblis-dev`/`intel-mkl` is the no-build fallback); CRAN for the R packages | log every BLAS/LAPACK call (routine, dims, µs) a workflow makes; swap backends for A/B |
| | Google Benchmark / nanobench / Catch2 benchmark | apt `libbenchmark-dev` / src | C++ microbench harness with counters, CV reporting, and `--benchmark_filter`; a better home for the `benchmark/*.cpp` harnesses than hand-rolled `std::chrono` loops |
| **R-side and Python-side profilers** | `Rprof(line.profiling=, memory.profiling=, gc.profiling=)` + `summaryRprof`, `profvis`, `proftools` (`flameGraph`, `hotPaths`, `callSummary`) | base / CRAN | R-level hot paths |
| | `jointprof` / `gprofiler` (R + native frames in one stack) | src (r-prof) | the only way to see C++ frames *under* the R frames that call them — settles the "is it R or C++" question directly |
| | `bench::mark(memory=TRUE)`, `bench::press` (n/p grids), `microbenchmark`, `RcppClock`, `tictoc` | CRAN | paired timings with allocation and GC counts; grids for scaling sweeps |
| | `atime` (asymptotic complexity curves + reference lines + version comparison) | CRAN | fits and plots `time ~ n` per function and flags complexity regressions across git refs |
| | `touchstone` | CRAN | CI benchmark PR-vs-main with statistical comparison (the regression gate for TODO-135) |
| | `Rprofmem`, `profmem`, `lobstr::obj_size`, `memuse`, `gcinfo(TRUE)`, `tracemem` | base / CRAN | R allocations and copy-on-modify surprises |
| | Python: `py-spy record --native`, `scalene` (CPU + memory + copy volume), `cProfile`+`snakeviz`, `line_profiler`, `memray`, `pyperf`, `pytest-benchmark`, `asv` | pip | for the Python bindings (TODO-165/179); `scalene`'s copy-volume column catches NumPy→Eigen array copies directly |

#### 8.0.1 Installing the roster

`profile/install_perf_tools.sh` installs everything above that can be installed unattended — apt (package names verified against Ubuntu 26.04 on 2026-08-22), pip, CRAN/GitHub R packages, and the git-clone tools (pmu-tools, FlameGraph, uiCA, nanoBench, bloaty, ClangBuildAnalyzer, nanobench, flexiblas) — and prints the manual steps for the vendor/licensed ones (Intel VTune/Advisor via the oneAPI apt repo, Intel SDE, Intel Pin, AMD uProf, hotspot AppImage, magic-trace, Compiler Explorer, ERT). One command:

```bash
bash profile/install_perf_tools.sh            # everything automatable (~10–20 min incl. source builds)
SKIP_SRC=1 bash profile/install_perf_tools.sh # apt + pip + R only
```

`profile/verify_perf_tools.sh` checks every item above (apt, binaries, pip, R, source builds, vendor tools) and prints OK/MISSING per item. Steps after apt need no sudo (pip `--user`, R into the user library, source builds and npm into `~/.local`), so `ONLY=pip,r,src bash profile/install_perf_tools.sh` can be re-run freely; only the apt step and the Intel oneAPI packages need root. Three roster items turned out to be non-trivial on Ubuntu 26.04 and are handled in the script: `pprof` is no longer shipped in the gperftools deb (the perl script is fetched from the gperftools 2.15 source tree), `touchstone` and `RcppClock` are CRAN-archived (installed from GitHub), and `jointprof` needs `proffer` ← `RProtoBuf` ← `libprotoc-dev` (its own `Remotes:` field points at a dead repo, so `proffer` is installed from CRAN first).

Installation status on this WSL2 box (2026-08-23, after running the script, the sudo apt step for `libgit2-dev`/`libprotoc-dev`, and the oneAPI install; `profile/verify_perf_tools.sh`): **everything installable is installed** — every apt package and binary, all pip tools, all 19 R packages (`touchstone`/`RcppClock` from GitHub, `jointprof` from its GitHub tarball via base R because `remotes` trips on its dead `Remotes:` line), all source builds (bloaty, ClangBuildAnalyzer, flexiblas, uiCA with its uops.info tables, nanoBench, pmu-tools `toplev`, FlameGraph, speedscope, the gperftools `pprof` script), hotspot AppImage and magic-trace in `~/.local/bin`, and Intel VTune 2026.4 + Advisor 2026.0 under `/opt/intel/oneapi`. Not installed: Intel SDE (manual licensed download), AMD uProf (AMD hosts only). VTune on WSL2 specifically: the package's `sep5` kernel-driver build fails at install (no headers for the Microsoft kernel) and leaves a failing `sep5.service` — harmless, disable it with `sudo systemctl disable --now sep5.service`; user-mode collections (hotspots, threading; Advisor survey) need `sudo sysctl -w kernel.yama.ptrace_scope=0`; hardware-event collections (`uarch-exploration`, `memory-access`, `-knob sampling-mode=hw`) answer "cannot recognize the processor" in WSL2 and need bare metal. Usage notes: `~/.local/bin` on `PATH`; `source /opt/intel/oneapi/setvars.sh` for `vtune`/`advisor`; `export LD_LIBRARY_PATH=$HOME/.local/lib:$LD_LIBRARY_PATH` for the flexiblas built before the rpath fix; hotspot's AppImage needs `libfuse2` or `--appimage-extract-and-run`.

WSL2 caveats (measured on this box, 2026-08-22, kernel 6.6.87.2-microsoft-standard-WSL2, i7-9750H, 12 vCPU; the script prints the same list): `linux-tools-$(uname -r)` does not exist for the Microsoft kernel — use `linux-tools-generic`'s binaries under `/usr/lib/linux-tools/<ver>/` or the locally built `/usr/local/bin/perf`. What **works** in WSL2: `perf record`/`report`/`annotate` with hardware *and* software sampling, `perf stat` with `cycles`/`instructions`/`branches`/`branch-misses`/`cache-misses` and named events (`fp_assist.any`, `topdown-*`), `perf stat --topdown` (TMA level 1), `perf diff`, `bpftrace`/bcc uprobes (BTF is present), valgrind/heaptrack/DHAT, `llvm-mca`/OSACA/uiCA/SDE, `hyperfine`, and every R/Python-side profiler. What **does not**: PEBS precise events (`cycles:pp` errors), therefore `perf mem` and `perf c2c` ("memory events not supported"); Intel PT (no `intel_pt` PMU); `/dev/cpu/*/msr` (so `likwid-perfctr`, `turbostat`, `likwid-powermeter` cannot read counters/frequency/energy); RAPL powercap; cpufreq governor/turbo control; and reliable physical-core pinning (`taskset` pins to vCPUs that the Hyper-V scheduler may migrate, so the TODO-135/175 noise floor is higher here). So of the TODOs above, TODO-143's `c2c`/`mem` halves, TODO-166's nanoBench calibration, TODO-171's likwid roofline, TODO-175's turbostat/RAPL logging, and the Intel PT option in TODO-132 need bare-metal Linux; everything else, including the TMA and counter work in TODO-134/140, runs here. See "What bare metal adds" below for the cost of staying on WSL2.

#### 8.0.2 What bare metal adds (and what WSL2 already gives)

If all of Phase 8 were run only on this WSL2 box, what is lost:

| Lost on WSL2 | Which TODOs | How much it matters |
|---|---|---|
| PEBS precise sampling → no `perf mem` (load-latency per line), no `perf c2c` (false sharing) | 143, 170 (HITM check), part of 172 | Medium: false sharing in the OpenMP kernels is otherwise only detectable indirectly (scaling curves in 147 + `pahole` layout inspection). Precise skid-free attribution also makes `perf annotate` on tight loops noticeably cleaner on bare metal. |
| Intel PT exact traces | 132 (option), 151/152 (exact call sequences) | Low: `uftrace` and bpftrace uprobes give exact call counts/sequences without PT. |
| MSR access → no `likwid-perfctr` derived metrics (GFLOP/s, bandwidth), no `turbostat` frequency/temperature, no RAPL energy | 171 (roofline), 175 (machine-state/energy logging), 136(d) (AVX-512 downclock check — moot on this i7-9750H, which has no AVX-512) | Medium for 171: the roofline can still be drawn from Advisor's own FLOP counting or from `perf stat` FP-arith events (`fp_arith_inst_retired.*` are available through the `cpu` PMU here) plus a measured STREAM bandwidth; just more manual. 175's frequency/thermal logging is the real loss — you cannot prove a WSL2 benchmark was not throttled. |
| Governor/turbo control and physical-core pinning | 135, 175 and every paired A/B | Medium: the noise floor is higher and turbo state is uncontrollable, so small (<5%) deltas need more repetitions or bare metal to call. Large deltas (the 2–7× wins this document is full of) resolve fine here. |
| Host contention from Windows (Defender, indexer, browser) and Hyper-V vCPU scheduling | 147, 148 (thread scaling / oversubscription) | Medium: parallel-efficiency curves above ~6 threads on a 6c/12t laptop under a hypervisor are not representative of a user's workstation; measure the *shape* here, the numbers on bare metal. |
| VTune/Advisor hardware-event collections: confirmed unavailable on this box (VTune 2026.4 `uarch-exploration` → "not applicable to the current machine microarchitecture", `-knob sampling-mode=hw` → "cannot recognize the processor"; the `sep5` driver cannot be built or loaded for the WSL2 kernel); user-mode `hotspots`/`threading` and Advisor survey/roofline (software FLOP counting) work once `ptrace_scope=0` | 172 | Low–medium: the `uarch-exploration` tree is the one VTune feature that really wants bare metal; `toplev`/`perf stat --topdown` give TMA level 1 here. |

What WSL2 already gives, and therefore costs nothing to keep doing here: all sampling profiles, TMA level 1, every counter-based TODO that does not need PEBS or MSR (134, 140, 154's branch-miss check), every simulator/static/asm tool (133, 141, 142, 166–168, 170), every eBPF, R-side, Python-side, compiler-flag, and algorithmic TODO (137–139, 149–165, 173, 177–179). Practical split: do the discovery and the large-effect work on WSL2; reserve a bare-metal session (even a one-off on a Linux workstation or a cloud bare-metal instance) for 143's `c2c`/`mem`, 171, 175, the final confirmatory paired benchmarks of TODO-135, and the published thread-scaling numbers of 147/148.

#### 8.0.3 The hardware this laptop cannot show, and which AWS instance to rent for it

The bigger blind spot than WSL2-vs-bare-metal is *scale*: a 6-core/12-thread, single-socket, no-AVX-512 mobile chip (i7-9750H) cannot reveal NUMA first-touch/remote-memory penalties on two-socket servers (the OpenMP kernels allocate and compute with no placement awareness), scaling cliffs at 32–96 threads (reduction contention, `schedule(dynamic)` lock traffic, memory-bandwidth saturation that 6 cores never reach), AVX-512 code generation and its frequency licensing under `-march=native`, or threaded-MKL × OpenMP × forked-worker interactions at real core counts. Those are exactly the conditions some users' workstations and servers will have, and bare metal on *this* laptop would not show them either. One rented many-core bare-metal session covers all of it and, being `.metal`, also supplies everything WSL2 lacks (PEBS for `perf mem`/`c2c`, Intel PT, `/dev/cpu/*/msr` for likwid/turbostat, RAPL, governor/turbo control, real core pinning).

**Recommendation: `c7i.metal-48xl` for the main session** (us-east-1 on-demand ≈ $8.5/h at time of writing — verify current pricing; spot is fine for this, it is a few hours of interactive work, not a reservation — do *not* buy a Reserved Instance or Savings Plan for a one-off):

| | `c7i.metal-48xl` (recommended) | `c7i.metal-24xl` (cheaper, no NUMA) | `c7a.metal-48xl` (AMD second data point) | `c7g.metal` (ARM, portability only) |
|---|---|---|---|---|
| CPU | 2 × Sapphire Rapids 8488C, 96 cores / 192 threads | 1 × Sapphire Rapids, 48c / 96t | 2 × EPYC 9R14 Genoa, 96c / 192t | 1 × Graviton3, 64 cores |
| NUMA | **2 sockets** — the only way to test first-touch / `numactl --membind` / remote-memory effects in 147/148 | single socket | 2 sockets (plus Zen4 CCX/L3 locality — a stricter NUMA-ish test) | single |
| SIMD | AVX-512 + AMX, DDR5 — tests `-march=native` AVX-512 codegen, `-mprefer-vector-width=256` vs 512, licensing downclock (136d) | same | AVX-512 (Zen4 "double-pumped") — different cost model, OSACA `ZEN4` / uiCA n/a | NEON/SVE — only for `EDI_PORTABLE` and OSACA's ARM port |
| Profiling | PEBS, Intel PT, MSR, RAPL, VTune/Advisor fully supported (`.metal` = no hypervisor) | same | AMD uProf + IBS precise sampling; no PT | `perf` only |
| Approx. on-demand $/h | ~8.5 | ~4.3 | ~9.8 | ~2.3 |
| Cost of an 8-hour session | ~$70 | ~$35 | ~$80 | ~$20 |

Why not a large *virtualized* instance (`c7i.48xlarge`, same CPU, slightly cheaper): Nitro VMs expose basic PMU counters on the largest sizes but not PEBS, Intel PT, MSR, or turbo/governor control — i.e. they reproduce the WSL2 limitations, just with more cores. The `.metal` premium is small and buys every tool in §8.0.

Session checklist, so the ~8 hours are not wasted on setup:
1. AMI: Ubuntu 24.04 LTS (or the same 26.04 as this box if available); `apt install linux-tools-aws` (perf for the AWS kernel); run `profile/install_perf_tools.sh` (pre-bake an AMI after the first run if a second session is likely); install R + EDI's build deps; `R CMD INSTALL` EDI *there* (bare metal is the one place a full build is expected).
2. `echo -1 | sudo tee /proc/sys/kernel/perf_event_paranoid`; `sudo modprobe msr`; `cpupower frequency-set -g performance`; decide SMT: run the 147/148 curves twice, once with SMT siblings offlined (`echo off | sudo tee /sys/devices/system/cpu/smt/control`) for clean per-core numbers and once with SMT on for the user-realistic picture.
3. `lstopo`, `numactl --hardware`, `likwid-topology -g`, `turbostat` log started — capture into `benchmark/_machine_state.sh` output (TODO-175) and commit the topology dump next to the results.
4. Run order for the day: TODO-143 (`perf c2c`/`perf mem` on the OMP kernels), 147/148 thread-scaling and oversubscription grids at 1…96 threads × `numactl --membind=0` vs interleave, 171 roofline (`likwid-bench` ceilings + `likwid-perfctr -g FLOPS_DP/MEM` markers or Advisor), 136(d) AVX-512 codegen/downclock A/B, 175 energy-per-fit via RAPL, then the confirmatory paired ABBA/BAAB re-run of the whole `benchmark_model_fits.R` table on a pinned core (TODO-135 reference numbers).
5. Copy `perf.data`/`likwid`/VTune result directories back (`aws s3 cp`) before terminating — they are the evidence the doc's entries cite.

**Is there one instance that covers every check in this section?** For everything Intel-side, yes: `c7i.metal-48xl` alone covers all five bare-metal TODOs (143 `c2c`/`mem`, 171, 175, the Intel PT option in 132, the published 147/148/135 numbers) *and* the scale/NUMA/AVX-512/threaded-MKL conditions this laptop cannot produce — there is nothing in the Intel column that needs a second machine. The only checks it cannot do are the ones that are a different ISA/vendor by definition: the AMD cost model and AMD uProf/IBS (`c7a.metal-48xl`) and ARM portability (`c7g.metal`). No AWS instance is both Intel and AMD, so "one machine for all checks" is impossible only in that trivial sense; the AMD and ARM rows are optional second data points, not prerequisites for any TODO. If you rent exactly one machine, rent `c7i.metal-48xl`.

If budget allows a second day, `c7a.metal-48xl` adds the AMD cost model (EDI users on Threadripper/EPYC) and AMD uProf IBS; `c7g.metal` is only worth it once an ARM build of EDI is in scope.

### 8.1 Close the methodology gaps this document has carried since day one

- [ ] **TODO-132: Line-level and inclusive-time attribution via a debug-symbol build.** The "Important caveat" at the top of this document — no clean line-level attribution, loop ranking is inferred — has never been fixed, and every `perf report` here is `--no-children` self-time, so a kernel whose cost is spread over many small callees (Eigen expression templates, inlined link functions) never shows up as a single hot symbol. Build once with `EDI_DEBUG_SYMBOLS=1` (adds `-g`; confirm `configure` also emits `-fno-omit-frame-pointer`, add it if not) into a throwaway lib dir, then re-run the Phase 6 harness with `perf record -g --call-graph dwarf -F 499` and `perf report --children --sort dso,symbol,srcline`, browse in `hotspot`, and produce one flamegraph per kernel (`perf script | stackcollapse-perf.pl | flamegraph.pl`, or `speedscope`). For one representative fit per family also capture an *exact* trace — `perf record -e intel_pt//u` + `perf script --insn-trace --xed` (or `magic-trace`) on Intel, `uftrace record -P.` on any host — so evaluation counts and per-call durations are measured, not sampled. Deliverable: a table of the top-5 *inclusive* frames per kernel alongside the existing self-time tables; any kernel where inclusive and self rankings disagree is a new finding.

- [ ] **TODO-133: Deterministic instruction/cache accounting with callgrind + cachegrind on the top-10 kernels.** `perf` at `-F 199` on a 1–5 ms kernel collects a few hundred samples per run; several entries above (TODO-69/70 rejections, the TODO-18 batch-exp reversal) were decided on differences within sampling noise. Run `valgrind --tool=callgrind --dump-instr=yes --collect-jumps=yes` and `--tool=cachegrind --cache-sim=yes --branch-sim=yes` once per kernel at the Phase 6 `n`/`p`, then `callgrind_annotate --auto=yes` (or `kcachegrind`) for exact per-line Ir/D1mr/LLmr/Bcm counts. Deterministic counts also give a noise-free "before/after" (`cg_diff`, `callgrind_annotate` on two runs) for micro-optimizations that are too small to resolve with wall-clock timing.

- [ ] **TODO-134: Top-down microarchitecture analysis per kernel.** The IPC-only interpretation table in the July 2026 section (IPC < 1.5 → "memory-bound") is a heuristic; TMA gives the real bottleneck class. Run `toplev.py -l3 --no-desc -- Rscript profile/edi_kernel_profiler.R <kernel>` (pmu-tools; falls back to `perf stat --topdown -r 10` if pmu-tools is unavailable) and record the L1 buckets (frontend-bound, bad-speculation, backend-bound split into memory vs core) and the L3 leaf per kernel. Cross-check the worst 5 with `vtune -collect uarch-exploration` (TODO-172). Frontend-bound kernels → TODO-160/169; backend-memory → TODO-141/143; bad-speculation → TODO-154; backend-core → TODO-166.

- [ ] **TODO-135: Benchmark noise floor and regression gate.** `benchmark/benchmark_model_fits.R` reports medians but there is no documented noise floor, no CPU pinning, and no automated regression check — a 10% regression in a 1 ms kernel would currently be invisible until someone re-reads `benchmark_model_fits.md`. (a) Measure the noise floor: run the full table 5× back-to-back pinned (`taskset -c 2`, `cpupower frequency-set -g performance`, turbo off, `OMP_NUM_THREADS=1`, `setarch -R`, a `turbostat` log running alongside to prove no throttling) and record per-kernel CV. (b) Add a gate — `touchstone` (PR-vs-main statistical comparison, built for R packages) or a hand-rolled threshold (fail if median > 1.15× baseline *and* outside the measured CI) — to the existing benchmark workflow. (c) Persist raw timings (not just medians) so the ABBA/BAAB paired-Wilcoxon methodology from TODO-129/130 can be applied retroactively; use `hyperfine --export-json` for the CLI-level runs and `perf stat -r 30 --table` + `perf diff` for counter-level before/after. (d) Consider porting the `benchmark/*.cpp` hand-rolled `std::chrono` harnesses to Google Benchmark / nanobench, which report CV and handle warmup/repetition properly.
...
- [ ] **TODO-137: Vectorized `exp`/`log`/`log1p` via libmvec without `-ffast-math`.** TODO-68 dropped `-ffast-math` (correct — it changes NaN/Inf semantics the likelihoods rely on), but the *subset* `-fno-math-errno -fno-trapping-math` is semantics-preserving for this code (no `errno` checks, no FP traps) and is what GCC needs to emit glibc's `_ZGVdN4v_exp`/`_ZGVdN8v_log` SIMD variants for loops marked `#pragma omp simd` (or auto-vectorized at `-O3`). exp/log/log1p are the top symbol in most GLMM, hurdle, and ZINB profiles above, so this is potentially the largest remaining single lever. Audit: compile one hot file (e.g., `fast_weibull_frailty.cpp`, `fast_zero_augmented_poisson.cpp`) with those two flags plus `-fopt-info-vec-all` to see which loops now vectorize and which still report `not vectorized: function call` (Clang cross-check: `-Rpass-analysis=loop-vectorize`); confirm in the asm (`objdump` / `gcc -S`) that `_ZGVdN4v_exp` is actually called; A/B with the paired methodology; verify bitwise-identical results are *not* expected (libmvec has ≤4 ulp error) and decide whether the existing tolerance tests (`test-*-cpp-equivalence.R`) cover it. Note the n≥10⁴ caveat from the MKL `vdExp` experiment in TODO-18 — the win may be size-dependent, so test at n ∈ {200, 1000, 10000}.

- [ ] **TODO-138: Profile-guided optimization (PGO) A/B.** Never tried. Build with `-fprofile-generate`, run `benchmark/comprehensive_benchmark.R` as the training workload, rebuild with `-fprofile-use -fprofile-partial-training`, and run the paired benchmark table. PGO mostly helps branchy, call-heavy code (optimizer loops, link dispatch, ordinal thresholds), which is exactly what the GLMM engine is. Inspect what changed with `-fopt-info-inline` and `bloaty -- EDI.so.pgo EDI.so` (code-size delta). If the win is ≥10% on the inference-weighted total, add an `EDI_PGO=1` configure option and a `benchmark/pgo_train.sh` (the `.gcda` files would be build-machine specific, so this is a developer/benchmark option, not a CRAN default).

- [ ] **TODO-139: `-O2` vs `-O3`, and unity-build inlining effects.** `-O3` is assumed, never measured; `-O3`'s aggressive unrolling/vectorization sometimes loses to `-O2` on branchy scalar code by i-cache pressure. `EDI_UNITY=1` is the default — it changes inlining decisions across what used to be translation-unit boundaries and can interact with `-O3` either way. Run the 2×2 (`O2`/`O3` × unity on/off) on the comprehensive benchmark; record `.text` size per build (`size EDI.so`) and `L1-icache-load-misses` per kernel to explain any result. Also confirm what R's own `CXXFLAGS` contribute on the build machines (Debian/Ubuntu R adds `-fstack-protector-strong -D_FORTIFY_SOURCE=2 -Wformat`; `-fstack-protector-strong` costs a few % on small hot functions) — `R CMD config CXXFLAGS` — and decide whether `override CXXFLAGS` should strip or keep them.

- [ ] **TODO-140: Subnormal / FP-assist audit.** GLMM quadrature and hurdle/ZI likelihoods evaluate `exp()` of large-negative arguments routinely, producing subnormals that cost ~100× a normal FP op via microcode assists. Count them: `perf stat -e assists.fp` (Intel) / `fp_assist.any`, or `likwid-perfctr -g FLOPS_DP` which exposes the assist counters on most uarches. Where non-zero, the fix is algorithmic (clamp `exp` arguments below −708 → 0, or skip quadrature nodes whose log-weight is < lse − 40), not FTZ/DAZ — setting MXCSR flags inside an R process is not acceptable.

- [ ] **TODO-166: Assembly-level analysis of the hot inner loops with `llvm-mca`, OSACA, and uiCA.** No instruction-level *static* analysis has ever been done here; `perf annotate` shows where samples land but not *why* a loop runs at the cycles/iteration it does. For the ~10 hottest inner loops (GLMM quadrature node loop in `_glmm_engine.h`, Weibull-frailty exp loop, ZINB/ZAP observation loop, ordinal CLMM threshold loop, pair-distance loop, the `fast_digamma`/`fast_trigamma`/`fast_log1pexp` bodies): extract the loop's asm (`gcc -S -fverbose-asm -O3 -march=native` on the file, locate the block `perf annotate` marks hot), then run (a) `llvm-mca -mcpu=native -iterations 200 -timeline -bottleneck-analysis` for port pressure, resource bottleneck, and the timeline; (b) `osaca --arch <uarch>` for port binding **plus loop-carried dependency** analysis (the serial `lse`/accumulator chains that `llvm-mca` under-reports); (c) `uiCA.py` on Intel hosts for a cycle-accurate number. Compare predicted cycles/iteration to the measured value (`perf stat` cycles ÷ iterations from TODO-151's counters): a match means the loop is compute/dependency-bound and the analyzers' critical path tells you what to break up (multiple accumulators, interleaving two groups' quadrature); a large gap means memory stalls the analyzers cannot see → TODO-143. Also grep the asm for `cvtsi2sd`/`cvttsd2si` (int↔double churn from `int` loop indices mixed with doubles), `vzeroupper` in inner loops (AVX/SSE transition penalties from mixing Eigen SIMD and scalar libm), and `call *@plt` (TODO-169).

- [ ] **TODO-167: Dynamic instruction-mix and ISA-usage audit (Intel SDE / valgrind lackey / objdump).** `sde64 -mix -- Rscript profile/edi_kernel_profiler.R <kernel>` gives the exact dynamic histogram: fraction of scalar-double vs packed-double (`*pd`) vs FMA vs AVX-512 vs x87/legacy, and per-function breakdown. This is the ground truth for "is this kernel actually vectorized" and quantifies what TODO-136/137 can still gain (if 95% of FP instructions are scalar, the SIMD ceiling is large; if 80% are already packed, move on). Fallback on non-Intel hosts: `valgrind --tool=lackey --basic-counts=yes` plus static `objdump -d | awk` instruction-class histograms. Report per kernel: % packed FP, % FMA, % loads/stores, % branches.

- [ ] **TODO-168: Compiler optimization-report sweep across all 108 files.** Nothing in this document has ever asked the compiler *why*. Build once with `-fopt-info-vec-missed -fopt-info-inline-missed -fopt-info-loop-missed` (GCC) and once with Clang `-fsave-optimization-record` → `opt-viewer.py` (HTML), and catalog every missed optimization inside a loop that `perf` says is hot: `not vectorized: control flow in loop` (→ TODO-154 branch-free layout), `possible aliasing` (→ `__restrict__` on raw pointer parameters, or `Eigen::Map` with `.data()` hoisted), `function call` (→ TODO-137 libmvec or an inlinable replacement), `unsupported data-ref` (strided `.row(i)`, → TODO-144), `loop costings not worthwhile` (→ revisit trip counts / `#pragma GCC unroll`), and `call … not inlined: size` (hot small functions that need `inline`/`__attribute__((always_inline))` or a smaller body). Two compilers because their vectorizers disagree, and the disagreement itself localizes the fragile loops.

- [ ] **TODO-169: Shared-object call overhead: PLT, symbol visibility, semantic interposition.** `EDI.so` is a shared library; by default GCC compiles every non-static function as interposable, so calls between EDI functions go through the PLT/GOT and cannot be inlined across TUs (the unity build hides some of this but not calls into Rcpp-exported functions or across the remaining TU boundaries). Measure: `objdump -d EDI.so | grep -c '@plt'`, which `@plt` symbols `perf report` attributes samples to, `readelf -r EDI.so | wc -l`, and `LD_DEBUG=statistics Rscript -e 'library(EDI)'` for relocation time at load (TODO-160). Then A/B `-fvisibility=hidden` (with `R_registerRoutines`/`useDynLib(.registration=TRUE)` already in place, only `R_init_EDI` needs to be visible — Rcpp's `compileAttributes` emits `RcppExport` with default visibility, so check it still links), `-fno-semantic-interposition`, `-fno-plt`, and `-Wl,-Bsymbolic-functions -Wl,-z,now`. Expect a small but free win on call-heavy paths and a faster `library(EDI)`.

### 8.3 Memory, allocator, and data layout

- [ ] **TODO-141: Hidden Eigen temporaries — `EIGEN_RUNTIME_NO_MALLOC` sweep.** TODO-9/13/51/69/70 chased allocations by reading `perf` for `_int_malloc`; Eigen can instead *assert* on any heap allocation inside a region. Build a debug variant with `-DEIGEN_RUNTIME_NO_MALLOC`, wrap each objective's `operator()`/`hessian()` body in `Eigen::internal::set_is_malloc_allowed(false/true)`, and run the smoke tests. Every assertion is a temporary in a hot path (aliasing products without `.noalias()`, `auto` capturing an expression, `.transpose() * v` materializing, `VectorXd::Zero(n)` re-created per step). Deliverable: a per-kernel list of temporaries per optimizer step, fixed where the count is > 0.

- [ ] **TODO-142: Allocator and malloc-count audit per kernel; allocator A/B.** Complement TODO-141 at the process level: `heaptrack Rscript profile/edi_kernel_profiler.R <kernel>` (allocation count/size/site, peak heap; `heaptrack_gui` flame graph) or `ltrace -c -e malloc+free+realloc` for a cheap count; `perf probe -x libc.so.6 malloc` + `perf record -e probe_libc:malloc` if uprobes are available. Then an `LD_PRELOAD` A/B of glibc malloc vs `libjemalloc.so` vs `libtcmalloc.so` on the comprehensive benchmark (pin with `taskset`, use `hyperfine` for the outer loop) — if a different allocator buys ≥5% across the board, the allocator-churn fixes above were treating symptoms and the kernels should pool their scratch buffers (one arena per fit, not per objective).

- [ ] **TODO-170: Allocation-lifetime, heap-timeline, and struct-layout audit (DHAT, massif, pahole).** DHAT answers a question heaptrack cannot: for each allocation site, how many bytes were *read* per byte allocated, how short-lived the block was, and whether it was ever read at all (`valgrind --tool=dhat` → `dh_view.html`; look at the "short-lived", "zero-reads", and "low access ratio" sort orders). Blocks allocated and freed inside one `operator()` call with a low access ratio are exactly the scratch buffers that should be members (TODO-9 pattern, applied systematically instead of per-finding). `valgrind --tool=massif --pages-as-heap=no` on one end-to-end `InferenceSuite` run gives the heap timeline and who holds the peak. Finally `pahole -C ZeroInflatedNegBin -C GLMMObjective … EDI.so` (on the TODO-132 `-g` build) and `clang -Wpadded` on the objective-functor classes: flag padding holes, hot members split across cachelines, and — critically for the OpenMP kernels — per-thread accumulators that are not `alignas(64)` and therefore false-share (pair with `perf c2c`, TODO-143).

- [ ] **TODO-143: Cache-miss and false-sharing counters.** `perf stat` runs so far captured instructions/cycles/branches only. Add `L1-dcache-load-misses,LLC-load-misses,LLC-store-misses,dTLB-load-misses` per kernel (or `likwid-perfctr -g L2CACHE`/`-g MEM` for the derived miss *ratios* and bandwidth), `perf mem record`/`perf mem report` for load-latency attribution to source lines on the worst ones, and `perf c2c record`/`perf c2c report` on the OpenMP kernels (`kk_compound_distr_parallel.cpp`, `fast_bai_parallel.cpp`, `kk_bootstrap_loop.cpp`, `compute_*_rand_bootstrap_parallel_cpp`) specifically to detect false sharing on per-thread accumulators / result columns written by adjacent threads. Any HITM on a shared cacheline is a fix (pad per-thread accumulators to 64 B or accumulate in thread-local and reduce once).

- [ ] **TODO-144: Systematic `.row(i)` / strided-access audit.** TODO-15/16 found that `m_X.row(i)` on a column-major matrix was a 5× regression in ZAP/ZINB. There are still 75 `.row(` sites in `src/`. Classify each: inside a per-observation loop (strided, fix via weight-vector + GEMV or by storing a row-major copy at construction) vs one-off setup (fine). Same pass for `NumericMatrix(i, j)` Rcpp accessors in inner loops (the pattern `pair_dist_helpers.cpp` was rewritten away from) and for `std::vector<std::vector<double>>` jagged storage in hot code. `cachegrind`'s D1mr per line (TODO-133) and `-fopt-info-vec-missed`'s `unsupported data-ref` (TODO-168) are the two tools that find these without reading all 75 by hand.

- [ ] **TODO-145: R↔C++ boundary copies.** 33 `as<Eigen::MatrixXd>`/`as<Eigen::VectorXd>` deep-copy sites vs 1060 `Eigen::Map`s. List the 33, and for each decide whether a `Map` (or `Map` + explicit `.eval()` only when mutation is needed) suffices. Also audit the *return* direction: `Rcpp::wrap` of large matrices, `Rcpp::List` with many named members built per bootstrap replicate, and `NumericMatrix` result objects allocated inside loops rather than once — measure with `bench::mark(memory = TRUE)` on the R side (`mem_alloc` column) for the 20 most-called exports, and `tracemem()` on the R objects passed in to catch R-side copies triggered by the call.

- [ ] **TODO-146: Peak memory per kernel and per workflow.** Nothing above reports RSS. Run `/usr/bin/time -v` (max RSS), `heaptrack` (peak heap + who), and `Rprofmem()`/`profmem` on the comprehensive benchmark and on an end-to-end `InferenceSuite` run; flag any kernel whose peak scales worse than O(n·p + p²) — e.g., materializing an n×n distance or n×B bootstrap matrix where a streaming computation would do. `numastat -p <pid>` on a multi-socket host to confirm allocations land on the node that computes on them.

### 8.4 Parallelism (OpenMP, BLAS threads, forked R workers)
...
- [ ] **TODO-150: `checkUserInterrupt` and `Rcpp::Rcout` placement.** 16 `checkUserInterrupt` sites; each is a cheap call but inside an inner loop it is a full R API round-trip and prevents vectorization of that loop. Confirm each is at the outer (replicate/iteration) level, never per-observation; same for any `Rcout`/`Rprintf` progress output in hot loops. `uftrace report` (call counts per function) or `bpftrace` `funccount` on `R_CheckUserInterrupt` makes the count per fit exact.

- [ ] **TODO-174: OpenMP runtime-level audit: races, affinity, and spin policy as performance.** (a) Run the parallel kernels' smoke tests under `valgrind --tool=drd --check-stack-var=yes` and a `-fsanitize=thread` (Archer, for OpenMP-aware reporting) build — a race that is "fixed" by adding `#pragma omp atomic`/`critical` in a hot loop is a performance bug waiting to be introduced, and a race that exists today is silently producing wrong sums. (b) `OMP_DISPLAY_ENV=VERBOSE OMP_DISPLAY_AFFINITY=TRUE Rscript …` once per machine class to record what libgomp actually does with placement by default in an R session. (c) `GOMP_DEBUG=1` / `perf sched` to count parallel-region entries per fit — a kernel that opens a region per optimizer iteration instead of once per fit pays fork/join hundreds of times. (d) `vtune -collect threading` for the definitive spin/wait/imbalance breakdown per region.

### 8.5 Algorithmic: work per fit, not cost per evaluation

- [ ] **TODO-151: Objective/gradient/Hessian evaluation counts per fit.** Every optimization above reduces the cost of one `operator()` call; none has measured *how many* calls a fit makes. Get the counts without touching the code first — `uftrace record -P. Rscript …` then `uftrace report -s call` (exact call counts and total/self time per function), or `bpftrace -e 'uprobe:/…/EDI.so:*GLMMObjective*operator* { @[probe] = count(); }'` on the installed library — then, if it is worth keeping permanently, a `static thread_local` tally exposed via an `EDI:::perf_counters_cpp()` export. Report value, gradient, and Hessian evaluations per fit for each kernel under the benchmark data, plus iterations and line-search backtracks. Kernels with high eval/iteration ratios (> 2–3) point at line-search tuning; high iteration counts at poor starting values (cross-reference the cold/warm-start audits: `audits/warm_starts.html`, `benchmark/exhaustive_cold_start_audit.R`), too-tight tolerances, or a poorly scaled parameterization (e.g., optimizing `theta` rather than `log theta`).

- [ ] **TODO-152: Duplicate evaluations at the same point.** `GLMMObjective` exposes both `value()` and `operator()` (value + gradient). Audit every optimizer driver for the pattern "call `value(x)` for the line search, then `operator()(x)` at the accepted point" — that recomputes the full quadrature a second time at the same `x`. `uftrace graph` / `uftrace replay` (TODO-151) shows the exact call sequence per iteration, so this is a read of the trace, not of the code. A last-point cache (store `x`, value, gradient; return cached on equality) is a one-line fix with a potentially 30–50% cut in quadrature work for any kernel that does it. Same check for Hessian paths that re-evaluate the gradient.

- [ ] **TODO-153: Adaptive Gauss–Hermite quadrature.** All GLMM kernels use a fixed `n_gh = 20` (`inference_ordinal_KK_combined.R:536`, engine default). Adaptive GH (center and scale the nodes per group at the group's conditional mode and curvature, as `lme4`'s `nAGQ` and `GLMMadaptive` do) reaches the same accuracy with 5–9 nodes — a 2–4× cut in the dominant `exp`/`log1pexp` count of every GLMM objective, on top of all the per-node micro-optimizations already retained. Prototype in `_glmm_engine.h` behind a flag, validate against the existing `lme4::glmer(nAGQ=20)` equivalence tests, and benchmark. This is the largest *algorithmic* lever left in the GLMM family.

- [ ] **TODO-154: Branch-free observation ordering.** ZINB/ZAP/hurdle objectives branch per observation on `y == 0`; ordinal objectives on the category; the `perf stat` branch-miss counts collected in July, `cachegrind --branch-sim=yes` per line (TODO-133), and the TMA bad-speculation bucket (TODO-134) identify which kernels pay for it; `-fopt-info-vec-missed`'s `control flow in loop` (TODO-168) tells you which of those also lost vectorization to the branch. Fix is layout, not arithmetic: permute observations once at construction so zeros are contiguous (and categories are contiguous), then run separate branch-free loops over each segment. Verify with the branch-miss counter dropping and the paired benchmark.

- [ ] **TODO-155: Linear-algebra choices at the actual sizes.** Audit every `.inverse()`, `.ldlt()`, `.llt()`, `ColPivHouseholderQR`, `FullPivLU` use: explicit `.inverse()` where only a solve is needed; pivoted QR where Cholesky suffices (SPD information matrices); full Hessian inversion each iteration where a rank-1 update or a fixed-size `Matrix<double,2,2>` would do (the Gaussian LMM m=1/m=2 finding in TODO-11 generalizes). Then see what actually reaches BLAS/LAPACK: `MKL_VERBOSE=1 Rscript …` (or `OPENBLAS_VERBOSE=2`) logs every `dgemm`/`dgemv`/`dpotrf`/`dsyevr` call with dimensions and time — confirm no small-p (p ≤ 20) products are being dispatched to BLAS where the call overhead exceeds the multiply (`EIGEN_USE_BLAS` is not set, good; the log tells you what R's own `solve`/`chol`/`qr` calls on the R side add). `flexiblas` lets the whole comparison NETLIB vs OpenBLAS vs MKL vs BLIS run without rebuilding R (TODO-176).

- [ ] **TODO-156: Early stopping for Monte-Carlo p-values and CIs.** Randomization/permutation p-values run a fixed `B` replicates. Sequential stopping (Besag–Clifford; or a CI on the p-value crossing the decision threshold) cuts the expected `B` by 5–10× for clearly-significant or clearly-null cases with controlled error. This is a statistical-policy change, so: audit where a fixed `B` is used, estimate the expected savings from existing benchmark workloads, and write it up as a proposal entry (opt-in argument), not a silent default.

- [ ] **TODO-157: Bootstrap-replicate reuse audit.** For every bootstrap/permutation loop: is the model matrix rebuilt per replicate (`model.matrix()` in R is expensive — `profvis` will show it), is the fit warm-started from the full-data estimate, is a per-replicate `Rcpp::List` result built and then post-processed in R when a single preallocated matrix would do, and is the design matrix copied into C++ once per replicate rather than once per call with an index vector (`tracemem()` / `bench::mark(memory=TRUE)` per replicate)? Cross-reference `audits/warm_starts.html` so this does not redo what that audit settled.

- [ ] **TODO-171: Roofline placement — know when to stop.** None of the micro-optimizations above was ever placed against the machine's ceilings, so there is no way to say "this kernel is at 60% of peak, stop" vs "at 5%, keep going". Get the ceilings once per machine (`likwid-bench -t stream_avx_fma` / `-t peakflops_avx_fma`, or the Empirical Roofline Tool, or `advisor --collect=roofline`'s built-in microbenchmarks), then per kernel either `advisor --collect=survey --collect=tripcounts --flop` (automatic per-loop roofline dots with the GUI/HTML report) or `likwid-perfctr -m -g FLOPS_DP` + `-g MEM` around a `LIKWID_MARKER_START/STOP` pair in the profiler harness (arithmetic intensity = FLOP / bytes). Deliverable: one roofline chart with every hot loop on it; loops far below both ceilings are latency/dependency-bound (→ TODO-166), loops on the bandwidth roof need layout/blocking (→ TODO-143/144), loops near the compute roof are done.

### 8.6 The R layer and end-to-end workflows

- [ ] **TODO-158: End-to-end profile of the user-facing workflows.** Every profile in this document is of a C++ kernel in isolation. Profile the full `InferenceSuite` / `compute_rand_bootstrap_*` / design-generation workflows end-to-end with `profvis`/`Rprof(line.profiling = TRUE)` (flame graph via `proftools::flameGraph`) and, in the same run, `perf record -g` on the whole R process (`perf report --sort dso` first: what fraction is `EDI.so` vs `libR.so` vs `libopenblas`/`libmkl` vs `libgomp` vs kernel). If `libR.so` (`bcEval_loop`, `R_gc_internal`, `Rf_eval`, method dispatch) is a large share — which the TODO-11 note about "R-overhead-bound" suggests for some paths — then no further kernel work moves the user-visible number and the next targets are in `EDI/R/`. Without a harness or rebuild, `bpftrace` uprobes on the installed `EDI.so` (`uprobe:…:*_cpp { @s[tid]=nsecs } uretprobe:…:*_cpp /@s[tid]/ { @lat[probe]=hist(nsecs-@s[tid]); delete(@s[tid]); }`) give per-export latency *distributions* (tails, not medians) and `funccount` gives call counts — a real workload weighting to replace the "Initial Kernel Weights" estimate at the top of this document.

- [ ] **TODO-177: One profile, both languages — combined R + native stacks.** TODO-158 as written gives two profiles (Rprof for R, perf for native) that have to be reconciled by hand. `jointprof`/`gprofiler` (r-prof on GitHub; `jointprof::joint_pprof`) record R and native frames in a single stack so a C++ `operator()` shows up *under* the R method that called it, and `pprof`/`profvis` can render it. Run it once on each major workflow; the single flame graph is the document's new "Initial Kernel Weights" section, with R-side cost attributed in the same picture. For the Python bindings, `py-spy record --native` does the equivalent (TODO-179).

- [ ] **TODO-159: R-side hot spots: dispatch, validation, and allocation.** From TODO-158/177's profile, rank R functions by inclusive time and check the usual suspects: R5/R6 method dispatch in tight loops (`$method()` lookups each call), `match.arg`/argument validation repeated per replicate, `data.frame` → `matrix` coercion per call, `model.matrix` rebuilds, `lapply` + `do.call(rbind, …)` result assembly, and GC pressure (`bench::mark(..., memory = TRUE)`; `Rprof(gc.profiling = TRUE)`; `gcinfo(TRUE)` count per workflow; `tracemem()` for unexpected copies). Also measure the result-key-store cache (hit rate per workflow; whether the keying itself — `digest`/`serialize` of a large object — costs more than it saves). `lintr` with the performance-oriented linters (`seq_linter`, `vector_logic_linter`, `unnecessary_lapply_linter`, `paste_linter`) as the mechanical first pass over 93k lines.

- [ ] **TODO-160: Package load time and binary size.** `system.time(library(EDI))` cold and warm, `strace -c -e trace=openat,read,mmap Rscript -e 'library(EDI)'` (file-touch storm from the lazy-load DB and `.so` deps), `LD_DEBUG=statistics` (relocation count/time for `EDI.so`, TODO-169), `R CMD INSTALL --byte-compile` status, size of the lazy-load DB, `.onLoad` work (class generation, registry builds, reading the tuning config). Plus `EDI.so` size (`size`, `bloaty -d symbols,sections EDI.so`) and `nm -C --size-sort EDI.so | tail -40`: the GLMM engine is a template instantiated per link × per family and the unity build can multiply that; a large `.text` shows up as frontend/i-cache stalls in TODO-134 and as slow load. If big, consider explicit instantiation in one TU or non-template link dispatch on cold paths, and `-ffunction-sections -Wl,--gc-sections` to drop unreferenced instantiations.

- [ ] **TODO-161: Forked-worker serialization costs.** For `mclapply`/fork-cluster paths (`parallel_fork_cluster_test_safety.md`, `benchmark_*_cores.R`): measure what is serialized to workers (`lobstr::obj_size` of the closure environment captured; `strace -f -e trace=write,read -c` on the pipes), whether large `X`/`y` are copied per task rather than inherited via fork CoW (`/proc/<pid>/smaps` `Private_Dirty` growth in the child; `tracemem`), and whether results are returned as large lists per task. A 1 ms kernel parallelized over 8 workers with 20 ms of serialization per task is a net loss; this is the kind of effect `tune_EDI_for_this_machine()` will otherwise mis-attribute to "parallelism doesn't pay on this machine".

### 8.7 Scaling, machine state, and automated checks

- [ ] **TODO-162: n/p scaling sweep to find super-linear kernels.** All profiles above sit at n ≈ 200–2000, p ≈ 4. Use `atime::atime()` (fits and plots `time ~ n` with reference lines for O(n), O(n log n), O(n²) and can compare git refs) or `bench::press()` over n ∈ {10², 10³, 10⁴, 10⁵} and p ∈ {2, 10, 50} (where feasible) for each export; flag any slope materially above the theoretical one (e.g., a Hessian path at 1.5 when it should be 1.0 → an n×n intermediate; a distance/matching kernel at 2.5 → a missing symmetry or blocking). Same for `B` (replicates) in resampling kernels, where the slope should be exactly 1. `atime` also doubles as a complexity-regression test in CI.

- [ ] **TODO-175: Machine-state controls and energy as a first-class measurement.** Every benchmark entry in this document reports wall time with no record of the CPU frequency, temperature, or contention it ran under — and `local_machine_optimization.md` already admits the shipped defaults were measured "possibly under contention". Standardize: a `benchmark/_machine_state.sh` that captures `lscpu`, `lstopo --of txt`, `cpupower frequency-info`, `cat /sys/devices/system/cpu/intel_pstate/no_turbo`, `sensors`, and starts a `turbostat --interval 1 --show Core,Avg_MHz,Bzy_MHz,PkgTmp,PkgWatt` log alongside every benchmark run, so a result can be rejected when MHz dipped or temperature climbed; `taskset`/`cset shield` + `setarch -R` for A/Bs of small diffs; `stress-ng --cpu $(nproc) --timeout 600` deliberately during one run to quantify how much the "under contention" caveat moves each policy table; and `perf stat -e power/energy-pkg/` (RAPL) so a "2× faster with 8 threads" result can be reported with its energy cost — a tuner that picks the fastest setting on a laptop should know it is also picking the hottest.

- [ ] **TODO-176: BLAS backend and call visibility.** The benchmark tables never state which BLAS/LAPACK they ran on, and several kernels (Gaussian LMM, OLS, Hessian inversions) spend real time there. (a) Record `sessionInfo()$BLAS`/`$LAPACK` and `RhpcBLASctl::blas_get_num_procs()` in every benchmark header; `benchmarkme::benchmark_std()` for a machine fingerprint. (b) `MKL_VERBOSE=1` / `OPENBLAS_VERBOSE=2` on one end-to-end workflow to enumerate every BLAS/LAPACK routine called, its dimensions, and its time (TODO-155's "does small-p go to BLAS" question, answered exactly). (c) Install `flexiblas` and re-run the comprehensive benchmark under `FLEXIBLAS=NETLIB`, `OPENBLAS`, `MKL`, `BLIS` without rebuilding R — this is the cheapest large-effect axis `tune_EDI_for_this_machine()` could ever report on, and the table tells users whether installing a faster BLAS matters for EDI at all.

- [ ] **TODO-172: Second-opinion passes with VTune / Advisor / uProf.** `perf` is sample-based and symbol-level; the vendor tools bring three things it lacks. (a) `vtune -collect uarch-exploration` and `-collect memory-access` on the 10 hottest kernels — a GUI-grade TMA tree per function plus per-source-line load latency and bandwidth, cross-checking TODO-134/143. (b) `vtune -collect threading` / `-collect hpc-performance` on the OpenMP kernels — spin time, wait time, imbalance per region, and vectorization % per loop (TODO-147/174). (c) `advisor --collect=survey` + `--collect=dependencies` on the hot loops — Advisor's Vectorization Advisor states per loop whether it vectorized, why not, and what the estimated gain is, and the dependencies analysis *proves* a loop has no loop-carried dependency so a `#pragma omp simd` / `__restrict__` can be added with confidence (TODO-137/168). On AMD hosts, `AMDuProfCLI collect --config ibs` gives precise-event sampling for the same purposes.

- [ ] **TODO-173: eBPF production observability without rebuilds.** Complementing TODO-158: install `bpftrace`/`bcc-tools` and write three one-liners against the installed `EDI.so` that run in any live R session (including a colleague's, or CI): `funccount 'EDI.so:*_cpp'` (which exports a workflow actually calls and how often — workload weighting), `funclatency -u EDI.so:fast_logistic_glmm_cpp` (latency histogram with tails), and `offcputime -p <pid>` (time spent blocked/waiting — forked-worker pipes, libgomp barriers, page faults). Zero overhead when not attached, no harness, no `-g` build. Document them in `profile/` next to the perf scripts.

- [ ] **TODO-163: Static performance lint pass.** Cheap, mechanical, never done: `clang-tidy -checks='performance-*,modernize-pass-by-value,readability-redundant-*'` over `src/` (catches pass-by-value `Eigen::MatrixXd` parameters, `for (auto x : big_vector)` copies, `push_back` without `reserve`, `std::string` concatenation in loops); `cppcheck --enable=performance`; `gcc -fanalyzer`; a grep for `auto` bound to an Eigen expression (`auto y = A * x;` is an expression template re-evaluated on every use — a classic silent O(n·k) bug); `-Wextra -Wconversion -Wdouble-promotion -Wfloat-conversion` on a one-off build to spot double↔int churn in inner loops (the `cvtsi2sd` TODO-166 looks for in the asm); and one `-fsanitize=undefined` run of the test suite — signed-overflow and misaligned-access UB is exactly what the optimizer is licensed to assume away, and removing it sometimes lets loops vectorize. Each finding in a hot path gets a TODO; findings in cold paths are fixed in a single cleanup commit without benchmark entries.

- [ ] **TODO-164: Developer-loop efficiencies (compile and test time).** Not user-visible runtime, but efficiency nonetheless and the thing most in the way of doing the audits above: (a) `Makevars` has no header-dependency tracking (the TODO-130 follow-up recompiled 50 files for one header edit) — add `-MMD` dep files or at least a `make`-level rule so header edits rebuild only includers, and evaluate `ccache` (`ccache -s` hit rate after a week); (b) measure unity-build compile time vs per-file with `-ftime-report` (GCC) and `-ftime-trace` + `ClangBuildAnalyzer` (Clang: which headers/templates cost the most — almost certainly `RcppEigen`), and whether a precompiled header or `include-what-you-use` pruning helps; (c) rank the 288 testthat files by wall time (`testthat::ListReporter` timings, or `R CMD check --timings`) and see whether the slowest 10% can use smaller `B`/`n` or share fixtures — a faster suite makes the paired-benchmark-plus-full-test-suite discipline this document insists on cheaper to keep.

- [ ] **TODO-165: Python-bindings parity benchmark.** `benchmark/benchmark_model_fits_python.py` exists alongside the R table; the 33-file model-fitting core is now R-free (TODO-130). Run both on the same data and machine and compare per-kernel: any kernel materially slower from Python points at binding overhead (NumPy → Eigen copies, GIL around a parallel region, per-call `np.ascontiguousarray` of a Fortran-order array) rather than the kernel itself, and any kernel materially *faster* from Python points at R-side overhead that TODO-158/159 should pick up.

- [ ] **TODO-179: Python-side profiler pass for the bindings.** `py-spy record --native -o out.svg -- python benchmark_model_fits_python.py` (C++ frames under Python frames, no code change); `scalene` for CPU + memory + *copy volume* per line (its copy-volume column is purpose-built to catch the NumPy→Eigen/Fortran-order copies TODO-165 suspects); `memray` for allocation flame graphs; `pyperf`/`pytest-benchmark` for stable paired timings and `asv` if a Python-side regression history is wanted. Same ABBA/BAAB discipline as the R side.

- [ ] **TODO-178: Compile-time and binary-size profiling tools.** The tools for TODO-164 spelled out so they are not forgotten: `-ftime-report` per TU; `clang -ftime-trace` → `ClangBuildAnalyzer --analyze` for the most expensive headers, templates, and functions to instantiate; `include-what-you-use` to cut transitive includes; `ccache`/`sccache`; `bloaty -d compileunits,symbols EDI.so` and `bloaty -- old.so new.so` to see which TUs/templates grow the binary after a change; `nm -C --size-sort`; `-ffunction-sections -fdata-sections -Wl,--gc-sections` + `-Wl,--print-gc-sections` to list what the linker could drop.

### 8.8 A-priori payoff estimates (not measurements)

Nothing below has been measured — these are planning estimates from the code structure, from what the earlier phases of this document actually delivered (2–7× on the kernels they touched; ~1 in 4 attempted micro-optimizations rejected after measurement: TODO-28/29/37/41/51/68/69/70/116), and from the one end-to-end data point we have (TODO-11: the Gaussian-LMM estimate-only kernel was 9.6% of wall time; the other 90% was R). They exist so the suggested order can be read against expected payoff; every entry must be replaced by a paired-benchmark number before it is cited anywhere else. "Kernel" = speedup of the C++ kernel(s) the TODO touches; "end-to-end" = what a user running `InferenceSuite`/a bootstrap workflow would see.

| Group | TODOs | Kernel-level (where it applies) | End-to-end | Confidence / notes |
|---|---|---|---|---|
| Infrastructure & measurement | 132–135, 166–168, 171–173, 175–179 | 0× directly | 0× directly | They decide which of the rest is real. 164 (dev loop) is the exception: header-dependency tracking + `ccache` plausibly cut incremental rebuilds 3–10× and the test suite 20–40% — developer time, not user time. |
| Vectorized exp/log without `-ffast-math` | 137 (+136c) | 1.3–2.5× on exp/log-dominated loops at n ≥ 1000 (GLMM quadrature, Weibull frailty, ZI/hurdle, ordinal); ~1.0–1.2× at n ≈ 200 where memory passes eat the SIMD win (the TODO-18 `vdExp` lesson) | 1.1–1.5× on GLMM-heavy workflows | Medium. The single largest *per-evaluation* lever left, but it is size-dependent and may be rejected for the small-n regime the policy tables care most about. |
| Adaptive Gauss–Hermite | 153 | 1.5–2.5× on every GLMM objective (20 → 5–9 nodes, minus the per-group mode/curvature cost) | 1.2–2× on GLMM-heavy workflows | Medium-high. The largest *algorithmic* lever; validated approach in `lme4`/`GLMMadaptive`. Compounds with 137 because it removes exp calls rather than speeding them up. |
| Duplicate / excess evaluations | 151, 152 | 1.0× (if no duplication) to 1.4× (if value() then operator() at the same point is routine); tolerance/line-search tuning 1.1–1.3× | similar | Medium. Cheap to measure with `uftrace`; a one-line cache if present. |
| PGO, O2/O3, PLT/visibility | 138, 139, 169 | 1.05–1.2× combined, broad and free | same, broad | Medium. PGO's win concentrates exactly in branchy optimizer/link-dispatch code; 169 is 1–5%. |
| Eigen temporaries, allocator, boundary copies | 141, 142, 145 | 1.05–1.3× on Hessian/full paths (TODO-11 saw 21% of full-path time in tiny allocs); ≤1.05× from a different allocator; 1.0–1.1× from boundary copies | small | Medium-high that *something* is there; low that it is large. |
| Strided `.row(i)` sweep | 144 | 1.0× for most of the 75 sites (cold); 1.5–5× for any hot one (the two known cases were 5×) | depends on which kernels | Low-medium — the obvious ones were already fixed; this is a tail search. |
| Branch-free layout, linear-algebra choices, subnormals | 154, 155, 140 | 1.1–1.3× on ZI/hurdle/ordinal; 1.0–1.3× on large-p Hessians; 140 is ~1.0× normally but 2–10× on the rare pathological fit | small | Medium. |
| Parallelism: thresholds, schedules, oversubscription, RNG | 147, 148, 149, 150, 174 | Mostly *removing slowdowns*: kernels that are currently slower with threads at small n (1.2–3× back), BLAS×OMP×fork collisions on bootstrap workflows (1.5–4× back on machines where it bites), R-RNG bottleneck in permutation/bootstrap index generation (1.2–2×, and it becomes parallelizable) | potentially the largest *workflow-level* swing, but highly machine-dependent — exactly why `tune_EDI_for_this_machine()` exists | Medium. False sharing (143) is binary: 1.0× if absent, 2–4× on the affected parallel kernel if present. |
| Early stopping, replicate reuse | 156, 157 | 156: 3–10× fewer replicates for clearly-significant/clearly-null Monte-Carlo p-values (policy change, opt-in); 157: 1.2–2× on bootstrap loops that rebuild `model.matrix` or cold-start each replicate | same | 156 high-confidence on the math, but a statistical-policy decision; 157 partly already harvested by the warm-start work. |
| R layer | 158, 159, 161, 177 | n/a | **the swing factor**: if the TODO-11 ratio (kernel ≈ 10% of wall) holds for other light-kernel workflows, R-side fixes are worth 1.5–3× end-to-end while *no* further kernel work moves the number; if it doesn't, ~1.1× | Unknown — that is the point of 158/177. 160 (load time) is seconds at `library(EDI)`, once per session. |
| Scaling, static lint, Python | 162, 163, 165, 179 | 162: 1.0× or unbounded (finding a hidden O(n²) at n = 10⁵ is the whole payoff); 163: 1.0–1.1×; 165/179: binding-overhead removal only | — | Low-medium. |

**Rolled up (a priori):** compounding the medium-confidence items, the GLMM family (logistic/ordinal/Poisson/hurdle GLMM, clogit+GLMM, Weibull frailty) plausibly gains **2–4×** at the kernel level (153 × 137 × 152 × 138/169), the non-GLMM regression kernels **1.2–1.8×** (137 where exp-bound, 141/144/154/138), and resampling/parallel workflows **1.5–4× on machines where the current thread/BLAS/fork configuration is wrong and ~1.1× where it already happens to be right**. End-to-end for a typical `InferenceSuite` run: **~1.3–2.5×**, with the R-layer result (158/177) deciding which end of that range — and deciding whether the kernel work should continue at all. Expect, on this document's own track record, roughly a quarter to a third of the measured micro-optimizations (the build-flag, allocator, layout, and linear-algebra items) to be rejected after a clean A/B; the infrastructure TODOs exist precisely so those rejections cost an hour each, not a day.

### Suggested order

1. TODO-132, 133, 134, 135, 175 first — pure infrastructure (call-graph/`-g` build, deterministic counts, TMA, regression gate, machine-state logging); they make every later measurement trustworthy and cheaper.
2. TODO-167, 168, 166 — the generated-code audits (instruction mix, compiler opt-reports, `llvm-mca`/OSACA/uiCA on the hot loops): cheap, never done, and they tell you whether TODO-136/137 are worth the effort before you spend it.
3. TODO-136, 137, 153 — the three with the largest plausible headline wins (vectorized exp/log across all GLMM/hurdle/ZI kernels; adaptive quadrature).
4. TODO-158, 177, 173 — to find out whether user-visible time is even in the kernels any more, with one combined R+native profile and eBPF call weighting.
5. TODO-147, 148, 174, 176, 161 — parallelism and BLAS, ideally before `tune_EDI_for_this_machine()` ships, so its axes are the right ones.
6. TODO-171 (roofline) once 1–3 are in, to decide where to stop; everything else as the measurements dictate.

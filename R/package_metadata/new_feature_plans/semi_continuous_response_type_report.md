...
> **Depends on:** gated decision; `sexp_removal_rcppeigen_conversion_spec.md` kernel conventions; `fix_inference_hierarchy.md` count-family components. (Global ordering: see `_master.md`.)
...
> interval-censored-survival plans). Read its shared-infrastructure citations through the new component
> names: `InferenceCountLikelihood` family bases are the `CountLikelihoodPlumbing`
> (and related count) components, `InferenceAsympLikStdModCache` is the
> `StandardModelCache` component, and "directly reusable" means listing the
> component in `components = c(...)` via `define_inference_class()`. The
> feasibility verdict is unchanged.
...
- [ ] TODO-2: Stage 1 — generalize Pattern A's abstract base (or write a lightweight sibling) — under the completed hierarchy this means a shared component for the two-part likelihood, not a new base class.
- [ ] TODO-3: Stage 2 — new C++ kernel(s) for the two-part / Tweedie-style fits, following the `EDI_CORE_ONLY`/`edi::ResultMap` conventions from the completed SEXP-removal spec.
- [ ] TODO-4: Stage 3 — concrete classes via `define_inference_class()` (components: `Wald`, `LikelihoodTests`, `StandardModelCache`, count-likelihood components as applicable).
- [ ] TODO-5: Stage 4 — `SimulationFramework` support (semi-continuous DGP).
- [ ] TODO-6: Resolve the point-mass-vs-censoring modeling question ("The Main Conceptual Problem") with the user before Stage 2.

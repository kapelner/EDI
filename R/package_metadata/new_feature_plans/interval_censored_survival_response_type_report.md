...
> **Depends on:** superseded by `interval_censored_survival_response.md` — its one remaining TODO (second-wave decision) depends on that plan's Tier 2/3 state. (Global ordering: see `_master.md`.)

> **Superseded (2026-08-14).** The staged plan this report recommended has been
> **implemented** — see `finished_features/interval_censored_survival_response.md`
> (the implementation plan this report motivated) for what actually landed:
> `Design` now stores the response as `y`/`y_L`/`y_R` bounds (the `(y, dead)`
> schema described throughout this report is the pre-migration form, retained
> internally only via the `get_effective_time()`/`get_effective_dead()`
> accessors for exact/right-censored data); the Weibull AFT engine fits
> left-/interval-censored data natively via a `log(S(y_L) - S(y_R))` likelihood
> term in C++; Cox/KM-family classes delegate general censoring to `icenReg`;
> and `InferenceSurvivalLogRank`/`InferenceSurvivalGehanWilcox` dispatch to
> `interval::ictest()`. This report is kept as the historical feasibility
> analysis; read its "cannot represent an interval today" statements as
> describing the pre-migration state, and note `InferenceAsympLikStdModCache`
> is now the `StandardModelCache` component under the completed
> `fix_inference_hierarchy.md` shallow hierarchy.
...
- [ ] TODO-1: Make a decision about whether to pursue the second-wave items this report deferred — ask the user: the semiparametric NPMLE/Turnbull path, `InferenceSurvivalStratCoxPHRegr`'s icenReg strata delegation, and the remaining engines listed as blocked in `interval_censored_survival_response.md`'s Tier 2/3 sections (which is where any such work should be tracked; this report is the superseded feasibility analysis).

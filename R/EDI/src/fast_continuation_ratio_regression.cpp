...
static ContinuationRatioAugmentedData build_continuation_ratio_augmented_data(const Eigen::Ref<const MatrixXd>& X,
	...
	for (int i = 0; i < n; ++i) {
		...
		for (int j = 0; j < rows_i; ++j, ++row) {
			...
			// z = 1 means "continued past this cut" (Y > j | Y >= j), not "stopped
			// here" -- this orientation keeps a positive coefficient meaning "pushes
			// toward higher categories", matching every other ordinal estimator in
			// the package (see fast_ordinal_regression.cpp's -x'beta convention and
			// the adjacent-category Pr(Y=j+1|...) numerator).
			z[row] = (yi_level == j) ? 0.0 : 1.0;

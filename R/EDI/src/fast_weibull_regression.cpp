...
public:
    ...
    double operator()(const Eigen::VectorXd& params, Eigen::VectorXd& grad) {
        ...
        double loglik = 0.0;
        Eigen::VectorXd d_eta(m_n);
        double d_ll_d_log_sigma = 0.0;

        for (int i = 0; i < m_n; ++i) {
            const double eta_i = m_eta[i];
            if (std::isfinite(m_y[i])) {
                const double log_yi = std::log(m_y[i]);
                const double w = std::min((log_yi - eta_i) / sigma, 700.0);
                const double ew = std::exp(w);
                loglik += w - log_sigma - log_yi - ew;
                d_eta[i] = (ew - 1.0) / sigma;
                d_ll_d_log_sigma += ew * w - (w + 1.0);
            } else {
        ...
        grad.setZero();
        grad.head(m_p).noalias() = -m_X.transpose() * d_eta;
    ...
    Eigen::MatrixXd hessian(const Eigen::VectorXd& params) {
        ...
        Eigen::VectorXd beta_weights(m_n);
        Eigen::VectorXd cross_weights(m_n);
        double diag_logsigma = 0.0;

        for (int i = 0; i < m_n; ++i) {
            const double eta_i = m_eta[i];
            if (std::isfinite(m_y[i])) {
                const double w = std::min((std::log(m_y[i]) - eta_i) / sigma, 700.0);
                const double ew = std::exp(w);
                beta_weights[i] = ew / (sigma * sigma);
                cross_weights[i] = (ew * (w + 1.0) - 1.0) / sigma;
                diag_logsigma += ew * (w * w + w) - w;
            } else {
                const CensoredRow r = censored_row_terms(i, eta_i, sigma);
                ...
                beta_weights[i] = -d2ll_deta2;
                cross_weights[i] = -d2ll_deta_dlogsigma;
...
// Portable core built directly on edi::ResultMap rather than
// _helper_functions.h's Rcpp-coupled
...
edi::ResultMap fast_weibull_regression_general_internal(
    ...
    WeibullAFTLikelihood fun(y, y_L, y_R, X);
...
Eigen::VectorXd get_weibull_regression_general_score_cpp(const Eigen::Map<Eigen::MatrixXd>& X,
    ...
    WeibullAFTLikelihood fun(y_vec, y_L_vec, y_R_vec, X);
...
List fast_weibull_regression_general_cpp(const Eigen::Map<Eigen::MatrixXd>& X,
    ...
    return edi::to_rcpp_list(fast_weibull_regression_general_internal(
...
#pragma omp parallel if(num_cores > 1)
    {
        ...
        Eigen::VectorXd y_exact_b(n), y_L_b(n), y_R_b(n);
...
#pragma omp for schedule(dynamic)
        for (int b = 0; b < nsim; ++b) {
            ...
            for (int i = 0; i < n; ++i) {
                if (dead_b[i] != 0.0) {
                    y_exact_b[i] = y_b[i];
                    y_L_b[i] = std::numeric_limits<double>::quiet_NaN();
                    y_R_b[i] = std::numeric_limits<double>::quiet_NaN();
                } else {
                    y_exact_b[i] = std::numeric_limits<double>::quiet_NaN();
                    y_L_b[i] = y_b[i];
                    y_R_b[i] = std::numeric_limits<double>::infinity();
                }
            }
            WeibullAFTLikelihood fun(y_exact_b, y_L_b, y_R_b, X_b);

...
NumericMatrix compute_rand_bootstrap_mean_diff_se_parallel_cpp(
	...
	const int*    i_ptr   = i_mat.begin();
	const int*    w_ptr   = w_mat.begin();
	// NumericMatrix is column-major: column b occupies [2b, 2b+1] (row 0 = t0, row 1 = se).
	double* out_ptr = result.begin();
...
#pragma omp parallel for schedule(static) if(use_parallel)
	for (int b = 0; b < nsim; ++b) {
		...
		const int n_C = n - n_T;
		if (n_T < 2 || n_C < 2) {
			out_ptr[2 * (size_t)b] = NA_REAL;
			out_ptr[2 * (size_t)b + 1] = NA_REAL;
		} else {
			const double mean_T = sum_T / n_T;
			const double mean_C = sum_C / n_C;
			out_ptr[2 * (size_t)b] = mean_T - mean_C;
			...
			out_ptr[2 * (size_t)b + 1] = (se_sq > 0.0) ? std::sqrt(se_sq) : 0.0;
		}
	}

...
ContinKKRobustRegrOneLikSource = list(
	public = list(
		...
		#'   computation to \code{compute_asymp_two_sided_pval()}, exposed under the explicit
		#'   \code{"wald"} name since this quasi-tier class supports only Wald-type asymptotic
		#'   inference.
		#' @param delta The null value of \eqn{\beta_T} to test against; 0 (the default) tests for
		#'   any treatment effect at all.
...
#' @R6method InferenceContinKKRobustRegrOneLik$approximate_randomization_distribution_beta_hat_T
#' @template rand-approx-distribution-beta-hat-t
NULL

#' @R6method InferenceContinKKRobustRegrOneLik$supports_rand_pval_for_incidence
#' @template rand-supports-pval-incidence
NULL

#' @R6method InferenceContinKKRobustRegrOneLik$compute_rand_two_sided_pval

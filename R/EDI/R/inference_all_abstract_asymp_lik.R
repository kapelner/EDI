...
InferenceAsympLik = R6::R6Class("InferenceAsympLik",
	...
	private = c(InferenceExtCIInversion$private, InferenceExtInformationMatrix$private, InferenceExtLikelihoodTestMemoization$private, list(
		...
		# Base-class default implementation of the likelihood-test extension
		# point: returns NULL, meaning "no likelihood surface available" --
		# the score, likelihood-ratio, gradient, and Bartlett-corrected
		# testing paths in this class hierarchy all check this method's
		# return value first and fall back to (or error out of)
		# non-likelihood inference when it is NULL. Concrete subclasses that
		# actually back a likelihood-tested model override this to return a
		# list describing the likelihood surface at the fitted point,
		# consumed by InferenceExtInformationMatrix,
		# InferenceExtLikelihoodTestMemoization, and (optionally)
		# InferenceMixinOffOptimumLikelihoodEval. Concrete classes typically
		# include X, y, j, full_fit, fit_null(delta, start),
		# extract_start(fit), score(fit), observed_information(fit),
		# fisher_information(fit), and neg_loglik(fit) -- all evaluated at a
		# completed fit object, never at an arbitrary theta.
		#
		...
		# parameter vector rather than a completed fit. Subclasses should
		# only add these if a genuine f(theta) native evaluator already
		# backs the *_information(fit) closures above (see e.g.
		# get_negbin_regression_hessian_cpp(X, y, theta) in
		# inference_count_negbin.R) -- never synthesize one via numerical
		# differentiation just to satisfy this contract. Classes that add
		# these two fields may splice in InferenceMixinOffOptimumLikelihoodEval
		# to get evaluate_penalized_neg_loglik_at() for free.
		#
		# Returns NULL in this base-class default; a list describing the
		# likelihood surface (see above) in overriding subclasses. This is a
		# private R6 method (an internal extension point, not part of the
		# public API), so it is documented as a plain comment rather than a
		# roxygen block -- roxygen2's R6 method-doc extraction only ever
		# looks at public_methods and can never attach a doc block to a
		# private method.

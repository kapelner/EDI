	...
	if (!is.na(n_val) && n_val < 200L) {
		...
		if (identical(operation, "rand") &&
				(grepl("^InferencePropBetaRegr$", inference_class, perl = TRUE) ||
				 ...
				 grepl("^InferenceSurvivalKKWeibullFrailtyIVWC$", inference_class, perl = TRUE))) return(FALSE)
		if (identical(operation, "jackknife") &&
...
golden_classes = c(
	...
	"InferenceSurvivalKKClaytonCopulaOneLik", "InferenceSurvivalKKStratCoxPHOneLik",
	"InferenceSurvivalKKWeibullFrailtyIVWC", "InferenceSurvivalGehanWilcox", "InferenceContinQuantileRegr",
	...
	"InferenceSurvivalKKClaytonCopulaIVWC", "InferenceOrdinalOrderedProbitRegr", "InferenceIncidRiskDiff",

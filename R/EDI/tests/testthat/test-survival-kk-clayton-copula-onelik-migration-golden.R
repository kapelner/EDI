...
# InferenceSurvivalKKClaytonCopulaOneLik migration (fix_inference_hierarchy.md,
...
# classes above). A registered component `SurvivalKKClaytonCopulaOneLik`
...
# non-exported `InferenceSurvivalKKClaytonCopulaOneLikLegacyRaw` R6 generator
...
make_survival_kk_clayton_copula_onelik_legacy_generator = function() {
	EDI:::InferenceSurvivalKKClaytonCopulaOneLikLegacyRaw
}
...
# InferenceSurvivalKKLWACoxPHOneLik segfault and the WeibullFrailtyOneLik/
# ClaytonCopulaIVWC instances of the same bug). Before the fix this produced
...
test_that("InferenceSurvivalKKClaytonCopulaOneLik migration produces identical outputs", {
	...
	migrated = InferenceSurvivalKKClaytonCopulaOneLik$new(des)
...
test_that("InferenceSurvivalKKClaytonCopulaOneLik is marked migrated", {
	EDI:::populate_inference_class_registry()
	metadata = EDI:::get_inference_class_metadata("InferenceSurvivalKKClaytonCopulaOneLik")
	expect_identical(metadata$parent, "Inference")
	manifest = EDI:::inference_hierarchy_migration_manifest_as_list()
	expect_identical(manifest[["InferenceSurvivalKKClaytonCopulaOneLik"]]$migration_status, "migrated")
})

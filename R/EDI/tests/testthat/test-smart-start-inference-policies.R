make_completed_fixed_design <- function(response_type, x, w, y, dead = NULL) {
	des <- DesignFixedTestFixture$new(n = length(y), response_type = response_type, verbose = FALSE)
...
test_that("smart_cold_start_default TRUE and FALSE agree across core optimization families", {
	...
	y_nb <- MASS::rnegbin(n, mu = exp(0.4 + 0.25 * w + 0.1 * x), theta = 2)
	...
	y_surv <- exp(1 + 0.3 * w - 0.15 * x + rnorm(n, sd = 0.15))

...
# InferenceContinKKGLMM migration (fix_inference_hierarchy.md, "KK And IVWC
...
# "ParametricLikelihoodBootstrap", "KKGLMM")` -- the same hybrid-state fix as
# InferenceOrdinalPropOddsRegr/InferenceOrdinalAdjCatLogitRegr earlier this
# stretch. No method bodies were touched (only pins/overrides/metadata
# added), so the legacy fixture below is the pre-migration file content
# verbatim (fixtures/legacy_contin_kk_glmm.R, copied from the committed
# source before this migration's edits, with the top-level class renamed to
# avoid colliding with the real migrated class) -- sourced into a child
# environment of the EDI namespace (not the namespace itself, which is
# locked against new top-level bindings) so `define_inference_class`/
# `InferenceParamBootstrap`/`KKGLMM` etc. resolve normally.
make_contin_kk_glmm_legacy_generator = function() {
	env = new.env(parent = asNamespace("EDI"))
	sys.source(testthat::test_path("fixtures", "legacy_contin_kk_glmm.R"), envir = env)
	get("InferenceContinKKGLMMLegacyOrig", envir = env)
}

contin_kk_glmm_golden_design = function(n = 24L, seed = 20260819L) {
...
test_that("InferenceContinKKGLMM migration produces identical outputs", {
	Legacy = make_contin_kk_glmm_legacy_generator()
	des = contin_kk_glmm_golden_design()
	legacy = Legacy$new(des)
	migrated = InferenceContinKKGLMM$new(des)
...
test_that("InferenceContinKKGLMM is marked migrated", {
	EDI:::populate_inference_class_registry()
	metadata = EDI:::get_inference_class_metadata("InferenceContinKKGLMM")
	expect_identical(metadata$parent, "Inference")
	manifest = EDI:::inference_hierarchy_migration_manifest_as_list()
	expect_identical(manifest[["InferenceContinKKGLMM"]]$migration_status, "migrated")
})

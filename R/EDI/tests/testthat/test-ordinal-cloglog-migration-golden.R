...
make_ordinal_cloglog_legacy_generator <- function(){
	source = EDI:::OrdinalCloglogLikelihoodSource
	R6::R6Class(
		"InferenceOrdinalCloglogRegrLegacy",
		lock_objects = FALSE,
...
test_that("InferenceOrdinalCloglogRegr migration preserves deterministic likelihood outputs", {
	legacy_class = make_ordinal_cloglog_legacy_generator()
	...
	expect_silent(expect_inference_migration_outputs_equal(
		...
		migrated_class = InferenceOrdinalCloglogRegr,
		design = des,
...
test_that("InferenceOrdinalCloglogRegr migration preserves seeded resampling outputs", {
	legacy_class = make_ordinal_cloglog_legacy_generator()
	des = inference_migration_complete_design("ordinal", n = 32L, seed = 20260817L)
	legacy = legacy_class$new(des)
	migrated = InferenceOrdinalCloglogRegr$new(des)
	method_calls = list(
		bootstrap = list(
...
test_that("InferenceOrdinalCloglogRegr migration preserves methods without new private-owner duplicates", {
	legacy_class = make_ordinal_cloglog_legacy_generator()
	before = list(
	...
	after = list(
		methods = inference_migration_public_methods(InferenceOrdinalCloglogRegr),
		dupes = inference_migration_duplicate_private_owners(InferenceOrdinalCloglogRegr)
	)

	expect_true(all(before$methods %in% after$methods))
	expect_true(all(
		inference_migration_expected_capability_methods("InferenceOrdinalCloglogRegr") %in%
			after$methods
	))
	expect_no_new_inference_migration_private_owner_duplicates(before$dupes, after$dupes)
})

test_that("InferenceOrdinalCloglogRegr passes the shallow migration gate", {
	EDI:::populate_inference_component_registry()
	EDI:::populate_inference_class_registry()
	metadata = EDI:::get_inference_class_metadata("InferenceOrdinalCloglogRegr")
	record = EDI:::mark_inference_class_migrated("InferenceOrdinalCloglogRegr")
	...
	expect_identical(
		EDI:::get_effective_components("InferenceOrdinalCloglogRegr"),
		c(
			"RandomizationTest",
			"RandomizationCI",

...
parse_args = function(args) {
  out = list(reps = 5L, out_dir = "R/benchmark/simd_matrix", keep_builds = FALSE)
  for (arg in args) {
...
if (!file.exists(worker)) stop("worker script not found: ", worker)
...
status = system2("git", c("-C", repo_root, "archive", "--format=tar", paste0("--output=", archive_tar), "HEAD", pkg_rel))
...
install_build = function(label, source_root, env = character()) {
  ...
  pkg_dir = file.path(source_root, pkg_rel)
  log_file = file.path(build_root, paste0("install_", label, ".log"))
  cmd = c("CMD", "INSTALL", "--preclean", "--no-multiarch", "--no-docs",
          "-l", normalizePath(lib, mustWork = TRUE), normalizePath(pkg_dir))
  cat("Installing build:", label, if (length(env)) paste0("[", paste(env, collapse = " "), "]") else "", "\n")
  status = system2("R", cmd, env = env, stdout = log_file, stderr = log_file)
  if (!identical(status, 0L)) stop("install failed for ", label, "; see ", log_file)
  lib
}
...
builds = list(
  ...
  current_no_vectorization = list(source = repo_root, env = c("EDI_DISABLE_VECTORIZATION=1")),

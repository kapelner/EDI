...
demote_markdown_headings = function(lines) {
...
python_benchmark_markdown_lines = function(path = "package_metadata/benchmark_model_fits_python.html") {
  ...
  if (length(body_start) == 0L || length(body_end) == 0L || body_start[1] >= body_end[1]) {
    return(c(
      "## Python Benchmarks",
      "",
      paste0("_Could not parse Python benchmark body from `", path, "`._")
    ))
  }

  body = html[(body_start[1] + 1L):(body_end[1] - 1L)]
  body = body[!grepl("^<nav>", body)]
  body = sub("^<h1>.*</h1>$", "## Python Benchmarks", body)
  body = sub("^<p><em>Generated: ([^<]+)</em></p>$", "_Generated: \\1_", body)
  body = gsub("^<h2([^>]*)>(.*)</h2>$", "### \\2", body)
  body = gsub("<sup>2</sup>", "^2", body, fixed = TRUE)
  c(body, "", paste0("_Source: `", path, "`._"))
}
...
combined_report_lines = c(
  ...
  r_benchmark_lines,
  "",
  python_benchmark_markdown_lines(),
  "",
  STYLE_BLOCK
)
...
if (requireNamespace("commonmark", quietly = TRUE)) {
  ...
  html_doc = c(
    ...
    paste0("<title>EDI v", edi_version, " R Baseline Benchmarks</title>"),
    "<style>",

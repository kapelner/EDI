"""Parity test for edi_kernels.fast_adjacent_category_logit against an
R-generated fixture.

This does NOT re-verify the statistical correctness of the adjacent-category
logit fit itself -- that's the same fast_adjacent_category_logit_internal
object code the R package's own test suite already exercises via
fast_adjacent_category_logit_cpp. What this test covers, which nothing on
...
X has NO intercept column (same convention as the other ordinal kernels --
the K-1 alpha thresholds serve that role); y is 1-indexed (values in
{1,...,K}), not 0-indexed.
...
so all the ordinal-link kernels share one directly-comparable dataset). Do
not regenerate this fixture casually -- if it needs updating, regenerate
from R and update the comment with the date/EDI version.
"""
import numpy as np
import pytest

from edi_kernels import fast_adjacent_category_logit
...
#   EDI:::fast_adjacent_category_logit_cpp(X, y)
R_B = np.array([0.402665437033552, -0.209608474408449])
R_ALPHA = np.array([-0.153443265853622, 0.24254951961509, 0.510252948350731])
R_NEG_LOGLIK = 400.88690697328


def test_matches_r_fixture():
    X, y = _synthetic_data()
    res = fast_adjacent_category_logit(X, y)

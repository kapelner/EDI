"""Parity test for edi_kernels.fast_logistic_regression against an
R-generated fixture.

This does NOT re-verify the statistical correctness of the logistic fit
itself -- that's the same fast_logistic_regression_internal object code the
R package's own test suite already exercises via fast_logistic_regression_cpp.
What this test covers, which nothing on the R side can: the pybind11
binding layer itself (argument marshaling, defaults, dict field conversion)
and the EDI_CORE_ONLY compiled path (this extension links fetched Eigen,
not the R build's RcppEigen).

The expected values below were computed once via:
    EDI:::fast_logistic_regression_cpp(X, y, estimate_only = FALSE)
in R, on the exact same synthetic dataset generated below with
numpy.random.default_rng(103).
"""
import numpy as np
import pytest

from edi_kernels import fast_logistic_regression

ATOL = 1e-9
RTOL = 1e-9


def _synthetic_data():
    rng = np.random.default_rng(103)
    n = 200
    X = np.column_stack([
        np.ones(n),
        rng.binomial(1, 0.5, n),
        rng.normal(size=n),
    ])
    beta_true = np.array([0.2, 0.8, -0.4])
    p = 1 / (1 + np.exp(-(X @ beta_true)))
    y = rng.binomial(1, p).astype(float)
    return X, y


# R reference, EDI 1.0.0, computed 2026-08-04 via:
#   EDI:::fast_logistic_regression_cpp(X, y, estimate_only = FALSE)
R_B = np.array([0.200514443588922, 0.997149366410844, -0.398899774793554])
R_NEG_LOGLIK = 120.612751517688
R_NUM_ITER = 5

# NOTE (2026-08-17): num_iter (renamed from "iterations") should be unchanged
# by the optimizer_diagnostics_report.md TODO-4 convergence-field rework --
# that rework only changes how `converged`/`hit_iteration_cap` classify a
# given exit, not which iteration the IRLS loop actually exits on. Re-verify
# this fixture (and that `converged` is still True here) against a fresh R
# run after the C++ change is compiled, since a step-size-tol exit is now
# reclassified by its recomputed gradient norm rather than assumed converged.


def test_matches_r_fixture():
    X, y = _synthetic_data()
    res = fast_logistic_regression(X, y, estimate_only=False)

    assert res["converged"] is True
    assert res["b"] == pytest.approx(R_B, abs=ATOL, rel=RTOL)
    assert res["neg_loglik"] == pytest.approx(R_NEG_LOGLIK, abs=ATOL, rel=RTOL)
    assert res["num_iter"] == R_NUM_ITER
    assert res["hit_iteration_cap"] is False


def test_result_shape_and_types():
    X, y = _synthetic_data()
    res = fast_logistic_regression(X, y, estimate_only=False)

    p = X.shape[1]
    assert res["b"].shape == (p,)
    assert res["mu"].shape == (X.shape[0],)
    assert isinstance(res["converged"], bool)
    assert isinstance(res["neg_loglik"], float)


def test_estimate_only_true_still_returns_b():
    X, y = _synthetic_data()
    res = fast_logistic_regression(X, y, estimate_only=True)

    assert res["b"] == pytest.approx(R_B, abs=1e-6, rel=1e-6)

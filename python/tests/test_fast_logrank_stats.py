"""Parity test for edi_kernels.fast_logrank_stats against an R-generated
fixture.
...
Same fixture data as test_fast_gehan_wilcox_stats.py (both stats were
computed from a single R session on one synthetic n=60 dataset, set.seed(456)).
"""
import numpy as np
import pytest

from edi_kernels import fast_logrank_stats
...
#   EDI:::fast_logrank_stats_cpp(w, y, dead)
R_SCORE = 0.71374057644385
R_VAR_SCORE = 10.5601795975526
R_BETA_HAT = 0.0475827050962568
R_SE_BETA_HAT = 0.218022479439416
R_N_TREAT = 30
R_N_CONTROL = 30


def test_matches_r_fixture():
    res = fast_logrank_stats(Y, DEAD, W)
...
def test_matches_scipy_style_chi_square_direction():
    # Independent sanity check: the score^2/var_score chi-square statistic
    # must be non-negative regardless of fixture values.
    res = fast_logrank_stats(Y, DEAD, W)

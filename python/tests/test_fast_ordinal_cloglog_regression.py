"""Parity test for edi_kernels.fast_ordinal_cloglog_regression against an
R-generated fixture.

This does NOT re-verify the statistical correctness of the proportional-odds
(cloglog link) fit itself -- that's the same fast_ordinal_cloglog_regression_internal
object code the R package's own test suite already exercises via
fast_ordinal_cloglog_regression_cpp. What this test covers, which nothing
...
The expected values below were computed once via:
    EDI:::fast_ordinal_cloglog_regression_cpp(X, y)
...
from edi_kernels import fast_ordinal_cloglog_regression
...
#   EDI:::fast_ordinal_cloglog_regression_cpp(X, y)
R_B = np.array([-0.499765483410496, 0.28926552255193])
R_ALPHA = np.array([-1.20071517888447, -0.117606205136146, 0.702589985182903])
R_NEG_LOGLIK = 398.906636415097
R_VCOV_DIAG = np.array([0.017293053991064, 0.00962067014054375, 0.00901623598956608,
                         0.0187471343166766, 0.0045954112833839])


def test_matches_r_fixture():
    X, y = _synthetic_data()
    res = fast_ordinal_cloglog_regression(X, y)
...
def test_estimate_only_omits_vcov():
    X, y = _synthetic_data()
    res = fast_ordinal_cloglog_regression(X, y, estimate_only=True)

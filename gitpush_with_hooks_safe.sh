...
while [ "$attempt" -le "$max_drift_attempts" ]; do
	if ! bash R/package_tests/drift_artifacts.sh regenerate; then
		echo "gitpush_with_hooks_safe: a package_tests/ generator FAILED (see output above) -- not pushing." >&2
		exit 1
	fi
